...
DG3 で得た「禁止挙動」（例: 「派手なグラデーション禁止」）は DONT.md にも転記する。
DESIGN.md の Do's and Don'ts は **視覚レベル** の禁止、DONT.md は **機能レベル** の禁止と棲み分ける。
...
DESIGN.md 自体のバージョンは YAML フロントマター先頭にコメントで記録する:
