...
「どこを読むべきかの索引（ルーティング表）」だけ。領域限定の規範実体・経緯・詳細手順は
所有元（SPEC.md / DONT.md / docs/）に一元化し、ルーティング表から遅延参照させる。
...
## Identity
- [何のプロジェクトか + 最上位原則。5 行以内]

## 常時規範
- [どのファイルを触っても効く規範だけを索引形式で（1 規範 1 行 + 所有元アンカー）]
- [判定: 純化 RL §3「CLAUDE.md から消したら、AI は grep 到達前に事故るか」— No なら領域規範としてルーティング表へ]

## ルーティング表
| 触る領域 | gate | 必読 | 検証 |
|---|---|---|---|
| [パス/領域パターン] | G-XXX | [SPEC 節 / docs アンカーを Grep] | [通るゲート・センサー] |
...
### 標準行: モデル選定の正本参照（G-MODEL、2026-08 追加）

L0 は CLAUDE.md 生成時、モデル選定の正本参照を標準で 1 行含める（Council `council-2026-08-05T14:34:36Z-f5fc45` 判定・人間合意）:

- ルーティング表型（M2 以上）: `| モデル選択・ティア判断・subagent のモデル指定 | G-MODEL | model-recommendations.md（DH 配布の選定基準） | lock-step: 本文にモデル ID・価格の直書きなし |`
- 罠索引型（M1）: 「G-MODEL: モデル選択・ティア判断は model-recommendations.md を参照。CLAUDE.md にモデル ID・数値を直書きしない（世代交代で陳腐化した値を参照し続ける事故に直結）」の 1 行

- 根拠: モデル ID・価格の直書きは実体二重定義（純化 RL §2 MUST）であり、モデル世代交代時の陳腐化事故を機械的に防ぐ
...
| 段 | 機構 | 発動条件 |
|---|---|---|
| 1. 記述 | ルーティング表の gate ID | 規格必須（ルーティング表型を採る全プロジェクト） |
| 2. Pre-flight 宣言 | DELIVERY.md「通過ゲート記録」に該当 gate と読んだ参照を明記 | M2 以上 |
| 3. 機械突合 | ルーティング突合センサー（diff × ルーティング表 × 記録、献上前必須） | 全 dev_mode |

検証は常に**実行主体のローカル**で走らせる（ローカル自律実行ファースト原則、`regime-assessment.md` §dev_mode 二軸注記）。GitHub Actions にしか存在しない検査を作らない。段 2/3 を持たないプロジェクトはルーティング表型でなく罠索引型（per-規範 1 行常駐）を維持する（純化 RL §7）。
...
生成物：
- CLAUDE.md（簡略版: Identity + 常時規範のみ。ルーティング表は領域分化が生じるまで任意）
- REGIME.md
...
```bash
# 総量: 20480 byte 超で WARN（DELIVERY に肥大警告 + 代謝提案を必須記載）
...
- 入力1: `git diff --name-only <base>...HEAD` の変更パス集合
- 入力2: CLAUDE.md ルーティング表の領域パターン列 → 該当 gate 集合
- 入力3: DELIVERY.md（または PR 本文）の「通過ゲート記録」に記載された gate ID 集合
- 判定: （該当 gate 集合 − 記載 gate 集合）≠ ∅ なら FAIL（未通過 gate を列挙表示）
- 実装: プロジェクト側 `scripts/` に単発 node スクリプト等で配置（先例型: kakuman `scripts/check-traps-sync.mjs`）。lock-step sensor（純化 RL §4）と並置する
- **実行場所は常に実行主体のローカル**（献上前必須）。CI に置くのはリモート実行型開発で runner 内エージェントが自走する場合のみ — その場合も「runner にとってのローカル」であり、独立した検査 workflow にしない（ローカル自律実行ファースト原則）
...
### この数を誰が何のために読むか（先に決める）

規則より先に読み手を書く。読み手のいない宣言は必ず腐るためである（本節の §バージョン履歴 が
v4.2 で止まったまま VERSION が 6 系まで進んだのが実例）。

| 読み手 | 読む目的 | 実際に読む粒度 |
|---|---|---|
| `dh-manifest.yml` の互換判定 | 更新が同一メジャー内か（再 sync 可か）を機械判定する | **major 部のみ**（`min_same_major_from`。minor を読む機械は現在存在しない） |
| 配布先の人間 / AI | 「自分の持つ DH に、あの機能は入っているか」を照会なしに判断する | minor まで |
| DH 自身の L0 | どの upgrade-spec が release 済みかを数える | minor まで |

**同一性の正本は版番号ではない**。「この DH は正確にどの内容か」は `UPDATE.md` の commit SHA ピンが担保する。
版番号に同一性まで負わせると SHA の劣化コピーになる。版番号が担うのは
**「約束が果たされた区切り」という意味だけ**である。

### 昇格の契機（いつ上げるか）

**その版の upgrade-spec の critical 機能がすべて master に入った時点で昇格する。部分実装では上げない。**

- **`破棄（理由を記載）` は充足として数える**。意図的に取り下げた critical（例 v6.12.0 F4 の次サイクル延期）を
  抱えた版が永久に昇格できなくなると、状態行に嘘を書く圧力が生まれるためである。
  破棄は当該 spec 本文に理由とともに明記されていることを条件とする
- **merge 時に上げる。遡及しない**。過去に遡って昇格させると、既に配布済みのツリーが
  古い番号を名乗ったまま新しい番号の内容を含むことになり、番号が内容集合を一意に指さなくなる
- standard / cosmetic の未実装は昇格を妨げない（critical のみが契機を決める）

### 昇格の実行（何を同時に書き換えるか）

以下を **同一 commit** で行う。1 つでも欠けると宣言間の drift になる。

1. `VERSION` ファイルを更新する（正本）
2. `GRAPH.yml` の `version:` を同じ値に更新する
3. 当該 upgrade-spec の状態行を `実装済み（PR #N、VERSION x.y.z）` に書き換える
4. **`history/CHANGELOG.md` に「この版を持つと何が違うか」を 1 行残す**。
   版番号が答えるべき問いは「実装がどこまで進んだか」ではなく「これを持つと何が違うか」であり、
   これを省くと正確だが空虚な数だけが残る

昇格の実行を人手の記憶に依存させないための常設チェックリストは**まだ無い**（Council vrsn01 条件 (8) の未了分）。
当面は本節の 4 項目を昇格 PR のレビュー観点として使い、常設化は次サイクルで扱う。

### 状態行の値域（upgrade-spec ヘッダの `> **状態: ...**`）

以下の 5 つ**のみ**を許す。値域外は機械検査で FAIL。

| 値 | 意味 |
|---|---|
| `L0 起草（人間レビュー待ち）` | 起草のみ。実装着手前 |
| `Council 諮問通過・人間の採否判定待ち` | 実装前 Council を通過し、人間の採否待ち |
| `実装中（F<n> 済 / F<m> 未）` | 一部が master に入った |
| `実装済み（PR #N、VERSION x.y.z）` | その版の critical がすべて master に入り、VERSION も昇格した |
| `破棄（理由）` | 採らないことを決めた |

**`VERSION` より大きい版の spec は状態行を必ず持つ**（まだ release されていない仕様は、
自分がどの段階かを宣言していなければならない）。VERSION 以下の歴史的 spec には遡及適用しない。

### spec 番号は起草時の予約であって release 番号ではない

`dh-upgrades/upgrade-spec-vX.Y.Z.md` の番号は**起草した時点で「次はこれ」と意図した予約**である。
一方 `VERSION` は**受け手が今何を持っているかの状態**を指す。両者は別種の名であり、一致する保証はない。

起草された順に実装されるとは限らないため、**未実装のまま release 線に追い越される spec が出る**
（実例: VERSION 6.15.0 昇格時点で v6.13.0 / v6.14.0 は未実装）。この扱いは以下に限る。

- 追い越された spec を実装するときは、**空いている次の番号へ付け替える**
- 付け替えは**書き換えではなく追記**で行う。旧番号を `drafted_as: v6.13.0` として本文に残す
  （不可逆な削除をしない = COLD ＝ archive ≠ delete と同型。「その時点で次の版だと意図した」化石を消さない）
- 付け替えの対象は **未実装かつ未参照の予約番号に限る**。
  **git 履歴 / PR 本文 / `history/` への遡及リネームは禁止**（不変参照を壊し、逆引き調査コストを増やす）
- 予約制の副作用として spec の乱発が起こりうる。**2 cycle 着手されない spec は棚卸しの対象**とし、
  実装するか `破棄（理由）` に倒すかを決める
...
> **本規則は 2026-09-05 まで守られていなかった**（v6.17.0 F1 の実測）。「何を major / minor と数えるか」も
> 「AI が判断してマイナーは自動記録する」も本節に既に在ったが、**いつ上げるかの契機が無かった**ため、
> PR #191 / #203 / #206 / #207 / #217 / #221 / #223 が VERSION 昇格なしで master に入り、
> `VERSION`（6.11.0）と `GRAPH.yml`（6.12.0）が乖離した。上の §昇格の契機・§昇格の実行 はこの欠落の是正であり、
> 機械検査（`harness-verifier/checks/declaration_coverage.py`）が宣言間の一致を強制する。
...
### バージョン履歴（v4.2 で凍結・以降の正本は `dh-upgrades/`）

> **凍結マーカー（v6.17.0 F1）**: 本一覧は **v4.2 までの記録として凍結**する。
> v5 系以降の版ごとの内容は `dh-upgrades/upgrade-spec-v*.md`（仕様）と `history/CHANGELOG.md`（変更ログ）が正本であり、
> ここに再掲すると実体の二重定義になる。**本一覧に v4.2 より後の項目を追記しないこと。**
> 凍結せず放置した結果、この一覧は VERSION が 6 系に進んでも v4.2 のまま 2 メジャー分死んでいた
> （＝新しい規則を置こうとしている当の節の中に、宣言の鮮度欠落の実例が在った）。
> 機械検査が凍結マーカーの存在と、v4.2 より後の項目が無いことを確認する。
...
> **状態: 時限規範**。下記 §賞味期限 の条件を満たしたら儀式から降ろす。
>
> 規範メタデータ:
> ```yaml
> stage: 全段階
> review_trigger:
>   - measured: 点検を 3 回実施し、3 回連続で「新規の型 C 所見 0 件」なら儀式から降ろす候補
>   - date: 2027-03-06
> ```

### なぜ点検か（Council `council-2026-09-06T14:30:00Z-slnt01`・jc 0.82・3 軸全会一致）

`delivery/ANALYSIS-silent-failure-2026-09-06.md` が定義した「静かな失敗」のうち、
**型 C（配線されているが通電していない）は原理的に決定論検知できない** — 「通ったこと」の記録が
そもそも残らないためである。痕跡を残す機構を作れば、その記録機構自体が静かに死ぬ無限後退に入る。

一方で実測は明快だった。**v6.17.0 の 8 項目のうち 7 項目が静かな失敗であり、8 項目すべてが
「点検」という単発の外部走査で発見された**（定常的な検知機構が拾ったものは 1 件も無い）。
点検は型 C に対する唯一の実績ある手段である。本節はそれに**リズムを与える**。

### 契機

以下の**いずれか**を満たしたとき点検を実施する。

| 契機 | 条件 | 根拠 |
|---|---|---|
| **版の区切り** | minor 昇格（`VERSION` の y が上がる）時 | 版は「何が違うか」の区切りであり、宣言と実体を突き合わせる自然な節目 |
| **期間下限** | 前回の点検から **60 日**超 | 昇格が止まった期間に点検も止まるのを防ぐ（Council 条件 b） |

> **60 日の根拠**（2026-09-06 実測）: 版の区切りは中央値 6 日・最大 22 日間隔。
> 60 日は「版の区切りが 3 回連続で止まった」状態に相当する。
> `signal-scan` の `REVIEW_TRIGGER_DAYS=90` をそのまま使わないのは、**90 日はこの周期では
> 一度も発火せず実質デッドコードになる**ため（同 (b)）。

### 範囲（Council 条件 c）

点検 1 回のコストは小さくない（初回実績: 29 エージェント / 所見 113 件 / 反証 12 件）。
**毎回全域を見ない。** 以下から 1〜2 面を選ぶ。

| 面 | 対象 | 型 C が出やすい理由 |
|---|---|---|
| **宣言層** | `GRAPH.yml` / `dh-manifest.yml` / `VERSION` / upgrade-spec の状態行 | 検査 8 が型 A を塞いだので、残るのは「宣言も実体も正しいが誰も通らない」経路 |
| **配布層** | `templates/` / `.claude/skills/` / `.claude/agents/` の配布物 | 届いていても読まれない（v6.17.0 F6 の RL 6 本が実例） |
| **儀式・手順層** | `ritual-protocol.md` / `UPDATE.md` / 各 SKILL.md のクロージング手順 | 書かれていても実行されない（`council-ctl.py export` が実例） |
| **CI・自動化層** | `.github/workflows/` / `scripts/` の検知器 | 走らなくなっても red にならない（gemini-review 4 ヶ月沈黙が実例） |

選び方: **前回と異なる面を選ぶ**。同じ面を続けると、見ていない面で型 C が育つ。

### 記録

点検の結果は `delivery/ANALYSIS-*.md` に残す（既存慣行）。**記録には上記の規範メタデータ形式で
`review_trigger` を宣言する** — これにより `scripts/signal-scan.py` の検知器 (c) が
「点検記録が更新されなくなったこと」を拾い、**点検が行われなくなったこと自体**を型 B の
検知として扱える（Council 条件 b の実装）。

> **この配線が依存している非対称**（Council 条件 d・警告として残す）:
> `scripts/signal-scan.py` の `fetch_review_trigger_files` は `git grep -l review_trigger` のみで
> **除外フィルタを持たない**ため `delivery/` を走査対象に含む。一方 `scripts/norm-scan.py` は
> `EXCLUDE_PREFIXES` で `delivery/` を明示除外する（分析文書は規範ではないため）。
> **両者の扱いは逆であり、本節の配線は前者の挙動に依存している。**
> `signal-scan` 側に除外を足すと点検の検知経路が黙って消える — **それ自体が型 C である**。
> 変更するなら本節を同時に読むこと。
>
> なお `signal-scan` は substring 一致のため、`review_trigger` を散文で言及するだけの
> 既存文書（実測 7 件中 6 件）も対象化している。これは既存の偽陽性源であり、本節が作ったものではない。

### 賞味期限（Council 条件 a / TR-4）

`telemetry-reflux.rules.md` TR-4 は「**常時発火する検知は形骸化する。検知器は絞って始め、
実測 1 ヶ月で見直す**」と定める。これは禁止則ではなく形骸化則であり、賞味期限という解を含む。

**点検を 3 回実施し、3 回連続で「新規の型 C 所見 0 件」なら儀式から降ろす候補**とする。
降ろす手続きは F2.6 の非対称原則に従い**一言承認で足りる**（追加側の重い手続きを削除側に課さない）。

制度化された点検が「やった」という記録だけを残して空洞化する危険は実在する
（DH はそれを「静かな失敗」と名づけたばかりである）。賞味期限はその危険への構造的な応答である。

### 採らなかった案と、その代償

**儀式 F1 に「今 cycle で一度も参照しなかった配布物はあるか」を問う 1 ステップを足す案は棄却した**
（Council で 3 軸とも棄却）。理由は 3 つ、いずれも独立に到達したものである。

- **認識論的に成立しない**: 想起は痕跡を要するが、不使用は痕跡を持たない。人は使わなかったものを
  思い出せない。答えられない問いを儀式に置くと人間は「特に無い」と答え続け、
  **その空答が「点検した」証跡として機能してしまう** = 型 C の新しい発生源になる
- **前例が実測されている**: `ritual-protocol.md` §F2.6 自身が「手順書に書くだけでは実行主体が無く
  空文化する。v6.1.0 の CTL 記録が同じ形で死んだ（発動 53 回に対し記録 1 件）」と記録している。
  走査器という実行経路を持たない口頭ステップは同じ形で死ぬ
- **ROI**: 儀式は既に 45 ステップあり、F1 の人間への質問は 1 問だけ。希少資源である人間の注意を
  毎 cycle 消費するのに、効果は人間の記憶依存

**代償**: cycle 単位の短期不作動は、次の版の区切り（または 60 日）まで見えないまま残る。
**この露出期間を許容するのが本判定である**（経営者軸の要請により明記）。
1 サイクル分の点検実績を見てから、提案 2 を再検討する余地は残す。
