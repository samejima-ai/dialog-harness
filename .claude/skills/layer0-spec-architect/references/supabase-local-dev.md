...
**本フローは既存の Supabase プロジェクトを持つ案件にのみ適用する。新規案件の供給元既定としては用いない。**

- **2026-09-11 観測**: Supabase 無料枠は **2 アクティブプロジェクト上限**。この上限は organization 単位ではなく
  **ユーザー単位で全 org を合算**する（Owner / Administrator を持つ全 org が対象）ため、org を増やしても回避できない。
  paused したプロジェクトは枠を消費しない
- **この値は腐る**（`dh-upgrades/upgrade-spec-v6.19.0.md` I-3）。供給元が単独で書き換えられる数値であり、
  規範ではなく**観測記録**として扱う。判断に使う前に現在値を確認すること
- 既存プロジェクトは**現状のまま使い続ける**（新規追加をしないという意味であって、
...
hosted Postgres 一般に通用する。

---

## 推奨発動条件（いつ L0 が薦めるか）

上記 §適用範囲 を満たす（＝既存の Supabase プロジェクトがある）ことが前提。その上で、
`subphase-selection.md` の S1（「データを保存する必要がありますか？ DB 使いますか？」）で **DB 使用あり** と判定され、かつ以下のいずれかに該当する場合に推奨提示する：

- 本番 DB に **既存の Supabase プロジェクト**を使う構成（§適用範囲 のとおり、新規に Supabase を選ぶ判断は本ファイルの対象外）
...
S1 で「DB 使う、たぶん Supabase」等と曖昧に答えた場合、**まず既存プロジェクトの有無を確かめる**（§適用範囲 により、本フローは既存案件専用）。`dialog-questions.md` S1 フォローアップの「もう使っている置き場所はありますか？」がこの確認にあたる。

- **既存の Supabase プロジェクトがある** → `ritual-protocol.md` E1 対応に従い「AI 推定 + 通知型」で薦める：

  > 「本番データを汚さないため、手元のローカルスタックで開発し、本番へは migration 経由で反映する流れを推奨します。違ったら指摘してください。」

- **新規案件** → 本フローは提示しない。供給元の選択に既定は置かず、対話で人間に選んでもらう
