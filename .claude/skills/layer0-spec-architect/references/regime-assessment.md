...
GitHub 利用は**保管庫軸**（remote push / PR 履歴 / レビュー記録の保存・可視化）と**実行環境軸**（Actions）の二軸に分離して判定する。**保管庫利用は Actions を要求しない** — 上表 `github_assisted` の「Actions 任意」は「Actions 不要（保管庫のみで完全動作）」と読み替える。検証・ゲート・センサー・マージ判断の一次実装は常に実行主体のローカルに置き、**CI にしか存在しない検査を作らない**（2026-08-05 ユーザー宣言）。
