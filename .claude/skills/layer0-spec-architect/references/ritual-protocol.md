...
2. 発火判定は走査器が行う: `date:`（期限超過）/ `cycles: N`（N cycle 相当の経過・日数近似）/
   ...
   `model_generation`（model-recommendations.md と `history/.model-generation.yml` の新しい方が当該規範より新しい・F7）。
...
3.5. **失効済み規範の COLD 移送（v7.0.0 Phase A F6）**: 走査器の「失効済みで購読に残っている規範」節に
   N 件あれば人間に **1 問**: 「失効済み規範 N 件が購読（HOT / WARM）に残っています。COLD へ移送しますか？
   （一括移送 / 個別に確認 / 後で）」。移送は `layer0-reindex-librarian` の次サイクル排泄で行う
   （`metabolism-regime.md` §2 昇降格「失効 → COLD」）。**宣言不完全**（`revoked_at` / `superseded_by` 欠落）が
   あれば移送より先に補完を問う（後継の逆引きを失ったまま退避しない）。失効の宣言自体はここでは作らない —
   3 の「廃止」で人間が選んだ規範に `status` = `revoked` を書くのが宣言（`dev-env-spec.md` §規範メタデータ §失効）。
   判定はしない（第 8 条）。
