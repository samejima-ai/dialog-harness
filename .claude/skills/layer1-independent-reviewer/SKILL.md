...
description: >
  ...
  **M2 以上のモードで L1 献上直後は、明示指示の有無に関わらず常時必須起動**（L1 から自動起動される想定）。
  ...
  「献上物の独立審査」「仕様と合ってるか確認」「動作確認して」「PR レビュー」（L1→reviewer の独立審査の意味）
  「反証して」「壊してみて」「Falsification」「このテストは信用できるか」等、
  検証・監査系の依頼でもトリガーする。
...
- **確証と反証の両輪**（v6.9.0 追加）。SPEC⇔成果物の一致証拠を集める確証だけでなく、「この実装が間違っていることの証明の試み」＝反証を明示的に課す。テストの Green は「現在存在するテストへの適合」しか保証しない（All Tests Passed ≠ Correct）ため、Green の保証範囲を毎回有限に明示する
...
- **反証チェック**（v6.9.0 追加）: critical 機能に対し counterexample 構築（挙動反証）・test-the-tests（テスト反証）・観測点の十分性検査（Oracle 反証）を実施し、「この PASS が保証しない範囲」を明示
...
5.10. 反証チェック（Falsification、v6.9.0 追加）
     - 確証（3-5）と向きが逆の検査。「壊れる入力・状態を探す」ことで Green の攻略（Reward Hacking 同型）と
       Test Oracle 不足を検出する。詳細プロトコルは `references/falsification-protocol.md`
     - **A. 挙動反証**: SPEC 記述から境界値・異常系・順序/並行の counterexample を導出して実行
       （critical 機能ごとに 1 件以上必須、上限目安 3 件のリスクベース打ち切り）
     - **B. テスト反証**: 既存テストが落ちうるかの検査（ミューテーション・スポットチェック＋原状復帰確認、
       恒真アサーション / over-mock / 自己整合テストの静的検出）。preserve 領域（archeo `no_modification`）では
       ミューテーション禁止・静的検出のみ
     - **C. Oracle 反証**: テストの観測点が SPEC の保証を観測できているか（HTTP 200 のみで DB / キュー状態を
       見ていない等）。不足は提起として記録
...
- **反証成立**（counterexample が SPEC 記載の保証を破る挙動を実証）→ **FAIL** として差戻し（提起のみ、訂正は L1）
- **反証不成立** → 試行記録を残して PASS 継続。ただし PASS の意味は「確証全通過 ∧ 試行した反証が全て不成立」であり、正しさの証明ではない（反証不成立 ≠ 正しさ）
- critical 機能で反証試行 0 件のまま PASS を返さない。試行不能な場合は理由を反証記録に明記した上で PASS（警告付き）
- 「この PASS が保証しない範囲」欄の省略はセンサー FAIL 扱い（省略検出時は VERIFICATION.md 不完全として再出力）
...
## 反証記録
- F1 ユーザー登録 (critical) / A: 二重送信 + 空メール境界で試行 → 反証不成立
- F2 ログイン (critical) / B: ロック閾値を 1 箇所改変 → テスト FAIL 確認 → 復元（diff clean） → 反証不成立
- この PASS が保証しない範囲: 並行リトライは単一プロセスでのみ試行 / F3 standard 機能は反証試行なし
