...
- **判定の運用基準**（2026-07-20 追加。原典の 1 行定義だけでは実際の発動で判断が割れるため、
  過去 85 件の遡及分類で用いた基準を明文化する）:

  | 値 | 原典定義 | 実運用の判定基準 | 迷ったときの決め手 |
  |---|---|---|---|
  | `C1` | 抵触判断 | 既存の罠・規約・原則・哲学に照らして「違反か / どう準拠するか」を裁く。哲学層への昇格可否も含む | **照合先の規範が既に存在する**なら C1 |
  | `C2` | トレードオフ | 複数案の拮抗から 1 つを選ぶ。スコープ・優先度・実装方針の選択 | **どれを選んでも規範違反にならない**なら C2（最頻出） |
  | `C3` | 不可逆操作 | 本番 DB 書込・migration・push・データ移送など revert が効かない操作の可否 | **失敗したとき revert で戻せない**なら C3 |
  | `C4` | 仕様矛盾解決 | 記述間の不整合・前提事実誤認・命名/表記の不統一・判定同士の衝突の解消 | **既存の記述 2 つ以上が食い違っている**なら C4 |

  境界の指針: C1 と C2 は「**規範との照合が主か、案の比較が主か**」で分ける。
  C3 は操作の可逆性のみで判定し、その操作を選んだ理由（トレードオフ）とは独立に付ける。
  C4 は「新しく決める」のではなく「既にある食い違いを解消する」場合に限る。

- **語彙マッチによる自動判定を実装しない**: 「適用」「投入」等の語は実装判断一般に頻出するため、
  正規表現ベースの分類は C3 を過剰検出する（実測で 85 件中 28 件が C3 判定され実態と乖離した）。
  これは §decision_category を機械導出しない と同根の failure mode（「満ちているが意味は空」）。
  **分類は発動時に判断者が付ける**（遡及付与する場合は AI が原案を出し人間が承認する）。
