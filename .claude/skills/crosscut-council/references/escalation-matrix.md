...
| **全ポジション** | **決定論検査（型 / lint / test / harness-verify）の FAIL** | **FAIL 確定**。Council・reviewer・drift・LLM judge の PASS では覆らない。優越は一方向（決定論 → 確率）で、逆は無い | 第2条 計算的解決最優先・第9条 §決定論の保全（v7.0.0 Phase A F5・arXiv 2609.02246: judge 信号は 11 通りに失敗し機械的棄却のみが回帰を捕捉） |
