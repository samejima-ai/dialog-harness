...
CTL の蓄積データは **`history/COUNCIL-LOG.md` を単一情報源として同期で導出する**。同期経路・`--prune` の警告・
個別 record の位置づけ・直接書き経路のスキーマ・事後評価の分離は [references/ctl-recording.md](references/ctl-recording.md)
が正本（v7.0.0 F10 で移送・内容不変）。発火主体は §クロージング手順。
...
- [references/ctl-recording.md](references/ctl-recording.md) — CTL 記録経路（v7.0.0 F10 で SKILL.md から移送）
