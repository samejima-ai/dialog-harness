...
| パターン | 射程外サブ要素 |
|---|---|
| ストラングラー・フィグ | (i) ルーティング層の設計 / (ii) ファサード経由の段階的トラフィック振り分け / (iii) 旧実装の段階的 decommissioning / (iv) フォールバック戦略 / (v) カナリアリリース併用 |
| Branch by Abstraction | (i) 抽象化レイヤーのインターフェース設計 / (ii) 新旧実装の段階的切替プロトコル / (iii) feature flag によるトグル / (iv) 切替後の旧実装 cleanup |
...
|---|---|---|
| 自動照合ループ（先行宣言 2） | 「新旧実装の並行実行 + 結果比較で意図保持を機械的に検証」という **照合メカニズム部分のみ** | ルーティング層 / ファサード / 段階的トラフィック切替 / decommissioning 計画 |
| Branch by Abstraction 言及（先行宣言 2） | 「抽象化レイヤー越しに新旧両者へ同入力を流す」という **概念的同型** のみ | feature flag / 段階的切替プロトコル / 旧実装の物理的 cleanup |

この区別を明文化することで、L1 実装者が「先行宣言 2 → ストラングラー / BBA 全パターン採用」と拡大解釈する誤りをガードする。
...
本宣言（先行宣言 4）と先行宣言 2/3 の記述に矛盾がある場合は **本宣言が優先**する。具体的には先行宣言 2 内の「Branch by Abstraction パターン」言及（本ファイル §先行宣言 2 第 1 項）は本宣言 (b) の境界線で限定解釈される。先行宣言 2 を更新する際は本宣言との整合を確認すること。
