...
| HOT / WARM → COLD（失効） | 規範メタデータの `status` が `revoked`（`dev-env-spec.md` §規範メタデータ §失効・v7.0.0 F6） | **次サイクルの排泄で移送**。結晶化完了の確認は失効宣言（人間の一言承認・儀式 F2.6-3.5）が代替。`superseded_by` を逆引きポインタとして残す。宣言不完全なら移送せず要再確認 |
...
- **失効規範は例外**（v7.0.0 F6）: `status` が `revoked` の規範は結晶化完了を待たず**次サイクルで排泄対象**。
  失効を宣言しても購読に残る記憶は害になる（append-only 0.210 < 記憶なし 0.309 < 明示失効 0.950 — arXiv 2608.07429）。
  列挙は `scripts/norm-scan.py`、移送の可否は人間（儀式 F2.6-3.5）
