...
> **二層モデルの要点**: Shift Left 基盤は**設計段階でエラー発生自体を防ぐ予防機構**であり、5層検出スタックには含まれない独立カテゴリ。5層検出スタックは**運用時に残存した問題を層ごとに検出する機構**で、第1〜5層で構成される。基盤 FAIL は「設計レビューで防げた問題が運用に流出した」メタ FAIL として独立集計する。
...
**エラーが発生する余地を設計段階で排除する**。本カテゴリは 5層検出スタックには含まれない独立カテゴリで、運用時の検出層ではなく**設計時の予防機構**である。最左・最優先。
