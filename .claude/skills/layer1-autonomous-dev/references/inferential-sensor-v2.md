...
**優越は一方向（v7.0.0 Phase A F5）**: 第 1・2 層（決定論）の FAIL は第 4・5 層（確率的 / 人間判断を除く LLM 判定）の PASS で覆らない。逆方向の上書きは無い。配線表は `crosscut-council/references/escalation-matrix.md` §1。
