...
> **二層モデルの要点（土地と建物の比喩）**: Shift Left 基盤は**土地**に相当し、全 Priority 共通で常に整備される設計時の予防機構。5層エラー検出スタックは**建物**に相当し、その土地の上に建つ 5 階建ての運用時検出塔で、第1〜5層で構成される。Priority は**建物の何階まで使うか**を決めるが、土地そのものは Priority に依らず常に整地されている前提（土地と建物は別ドメインで、階数に加算されない）。基盤 FAIL は「設計レビューで防げた問題が運用に流出した」メタ FAIL として独立集計する。
