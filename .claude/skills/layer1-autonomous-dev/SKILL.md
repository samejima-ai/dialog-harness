...
起動条件 `exists("delivery/refactor-intent-map.md") AND map.Meta.self_verification == "passed"` の成立時のみ第 4 軸「意図合致」を
起動する。preserve（`assertion: no_modification` 一次条件 + 承認テスト二次条件）/ restructure（新旧並行実行の自動照合）/
discard_and_redesign / 判定の集約は `references/intent-verification.md` が正本（v7.0.0 F10 で移送・内容不変）。
不在時は完全スキップ（後方互換・3 軸動作）。
...
- `references/intent-verification.md` — 意図合致検証の 4 軸プロトコル（v7.0.0 F10 で SKILL.md から移送）
