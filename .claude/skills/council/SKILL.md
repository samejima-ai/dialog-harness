...
description: >
  ...
  横断的に呼ばれる。
  **ユーザーが対話段階で「迷っている」と発話するケースでは起動しない** — それは
  spec-architect の対話で吸収する。L1/L2 実行中に「複数の実装案で迷った時」
  「どっちが正解か決められない時」「trade-off」「複数案が拮抗」「Council に諮る」
  「合議」「重み付き判定」「止揚」等の判断点が現れたら起動する。
