...
> `scripts/check-diary-safety.py` / `scripts/count-diary-references.py` / 退避先の `drafts/`）と、
...
儀式記録（レベル 2 / 検出 2 件）を `history/CHANGELOG.md` へ追記する。
**書き込みの時点は L1 献上時**（`.claude/skills/layer1-autonomous-dev/references/delivery-format.md`
§履歴層更新差分）であり、本仕様のマージ時点ではない ── 儀式プロトコルの定めどおり
F3 は**予告**に留まる（`.claude/skills/layer0-spec-architect/references/ritual-protocol.md` §F3）。
...
| F2-2 | **fail closed**。1 件でも検出したらコミットしない。下書きは **git 管理外の `drafts/` 配下**へ退避して人間へ回す（`.gitignore:14` で ignore 済み）。**tracked なディレクトリに書かない** ── `harness-verifier/reports/` は tracked であり、検出物をそこへ置くと「止めたはずの内容を作業ツリーに残す」ことになる |
| F2-2b | スキャン結果の報告は**件数と種別のみ**とし、検出した文字列そのものを出力・ログ・PR 本文へ書かない（報告経路が二次的な漏出経路になるのを防ぐ） |
