...
| D-2 | 観測 skill の prefix・命名・配置 | **checklist 評価は完了し、実装は `crosscut-quota-observer`（`crosscut-` prefix / Level A）で確定させた**（評価結果は同 SKILL.md §未充足項目）。ただし本行は「人間専管」であり、**人間の追認をもって確定とする**。追認前に merge すると記録と実態が食い違う（philosophy 第 6 条） |
