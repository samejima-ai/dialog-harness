...
> **実装済み（PR-E、2026-09-05）**: `check_pat` の有効性検査（`gh api user` が 401 なら `available=false` → skip）と
> 検知器 (e) `decide_workflow_silence` を実装。基準時刻は最終 run、無ければ workflow ファイルの最終 commit
> （新設直後の誤検知を避ける）。`bash -e` 下で `elif ! cmd; then` が errexit を起こさないことを実証で確認済み。
> **PAT の再発行（是正 1）と gemini-review の原因調査（是正 3）は未了** — 前者は人間専管（§実装しないもの）、
> 後者は D-7 の判断待ち。実装したのは「同じことが起きても red にせず検知する」機構のみ。
