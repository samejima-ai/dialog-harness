...
> **状態: 実装済み（PR #248 / #253 / #256 / #257 / #258、VERSION 6.17.0）**。本仕様の実装は escalation-matrix「規範文書改変」行
...
- **検査 8 の肥大**: ~~F1 / F2 / F4 / F6 を 1 ファイルに集約するが、4 種類の異なる宣言を 1 検査器が読むことになる。
  ...
  **【2026-09-07 完了 / v6.18.0 C-2】672 行・13 項目に肥大したため 4 モジュールへ分割した**
  （Council `council-2026-09-06T15:00:00Z-splt02` / jc 0.85 / 3 軸全会一致）。
  **分割基準は上記の「1 検査器 = 1 宣言ファイル」ではなく「1 検査器 = 1 つの欠落の型」を採った。**
  括弧書きの基準が実態と合わなかったためである（実測: F1 は 4 ファイル、F4 は 2 ファイルを読み、
  F3 は宣言を読まない = 実体走査のみ）。**予約された基準は実装前の視界で書かれる以上、
  実装後の実態と食い違いうる** — 予約を「そのまま従う指示」ではなく「再考の契機」として扱う。
  分割の決定的根拠は行数ではなく、**Council の 3 軸が独立に同一のバグ**（dict 重複キーによる
  F4 METRIC の消失・PR #263 で是正）**を発見した**こと。肥大が実際に見落としを生んだ実証である。
  分割後: `declaration_freshness`（鮮度）/ `declaration_coverage`（実体→宣言・実質）/
  `distribution_state`（実体の誤配置）/ `rules_index`（RL 索引）。共通ヘルパは
  `checks/_declaration_util.py` に集約（fallback の複製を禁ずる）。
  発火実績の棚卸しも同時実施 — **13 項目中 12 項目に実際の検出実績があり**、
  「空の器を整理する」状態ではなかった（唯一 0 件の検査 12 は新設時に是正済みゆえ）
