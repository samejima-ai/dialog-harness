...
- 5本柱原則と既存履歴層規約への整合性を、各Step実行前に確認すること
...
- 「削除」ではなく「退避」（`.claude/disabled/` へ）— 5本柱P4（情報メタボリズム）整合
...
追加層（本改修で新設）：
  ...
  思想検証（5本柱整合）※ v5.0.0 では placeholder
```
...
description: >
  実装が 5本柱原則と整合しているかを検証する横断機構（placeholder）。
...
- [ ] 仕様1〜4 の同型構造が 5本柱P1（フラクタル）を発現しているか
- [ ] CTL連動が 5本柱P2（再帰進化）を発現しているか
...
- 5本柱原則 → `.claude/skills/layer0-spec-architect/references/philosophy.md`
