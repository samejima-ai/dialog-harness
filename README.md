...
mkdir -p scripts && cp dialog-harness/scripts/council-ctl.py scripts/   # 任意: CTL ツール（無くても Council は直接 append で自動記録。init 不要 — record が未初期化なら自動コールドスタート）
# 任意: claude-review の tier 段階 Council をレビュアーに使う場合のみ（autonomous-drive が認識合わせで自動配備するので通常は不要）
# cp -r dialog-harness/.claude/agents .claude/   # review-* worker/persona 8 個。crosscut-council skill が前提
```
