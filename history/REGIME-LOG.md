...
- 判定日: 2026-09-11
- 判定者: ユーザー（`toriaezu.hide@gmail.com`）。判断キット
  `delivery/DECISION-KIT-version-promotion-2026-09-11.html`（PR #285）の 5 問への回答で確定
- AI 能力バージョン: claude-opus-5
- 改修主体: layer0-spec-architect 対話セッション（v6.18.0 = 時限の主体化 /
  v6.19.0 = Cloudflare 吸収 + 共有枠の観測）→ 同セッション内 L1 実装
- **minor 昇格の根拠**: 後方互換維持。`philosophy.md` の既存条の責務再定義・削除なし。
  参照リンクの削除・改名なし。規範の変更は既定の**降格**（Supabase を新規案件の既定から外す）と
  **追加**（Stack 12・プレイブック 2 本・観測 skill 1 本）に限られ、既存プロジェクトの
  動作条件を破壊しない。`dh-manifest.yml` の互換判定は major 部のみを読むため影響なし
- モード判定: **M2 標準**（S=1 実装対象が明確 / U=1 対話と Council で解消済み /
  R=2 DH は配布物ゆえ利用者プロジェクトへ波及し R≥2 で M2 強制。L2 閾値未達）
- 儀式記録: レベル 2（LC ≥ 1・機能追加）
- Council 記録: `council-2026-09-11T03:07:42Z-dt0911`（recommended C + 必須 4 条件 /
  judgment_confidence 0.35 で escalate_to_human → 人間が `agreed`）。
  最重量軸（哲学者 weight 5 = ΣW の 45%）が options 外の第 3 の道へ退避したため機構が判定を降りた構造

### 含む内容

**v6.18.0 由来**: C-1（`norm-scan.py`）/ B-1（期限条項付きの点検）/ B-3（`glossary.yml`）/
C-2（検査 8 の分割）/ C-4（検査の静かな無効化を塞ぐ）/ F0（版番号の自己適用）。
**v6.19.0 由来**: F1〜F5（Supabase 既定性の降格 / `crosscut-quota-observer` /
Cloudflare プレイブック / Stack 12 と観測の配線 / データ層の機械判定）。

**6.18.0 は v6.19.0 の内容も含む。** spec 番号が release 番号を追い越した状態であり、
`dev-env-spec.md` §spec 番号は起草時の予約 が想定する形（先例: v6.12.0 は VERSION 6.15.0 で
`実装済み` を名乗る）。v6.19.0 の状態行は自版 > VERSION ゆえ `実装済み` を名乗らない。

### 昇格に伴う 2 つの判定（いずれも人間）

1. **C-3 を `priority: standard` と判定した**。`upgrade-spec-v6.18.0.md` の 7 項目には
   `priority:` 表記が 1 件も無く、唯一未着地の C-3 が critical か standard か判定できなかった。
   standard とすることで規則「standard / cosmetic の未実装は昇格を妨げない」が適用され、
   C-3 未着地のまま昇格した。**残る 6 項目の priority 表記は補っていない**（今回の判定は C-3 のみ）
2. **v6.18.0 F0 §是正 step 2 の前提が覆った**。同項は「`VERSION` は 6.17.0 のまま据え置く。
   C-3 の実装完了時に 6.18.0 へ昇格させる」と書いていた。上記 1 により完了を待たず昇格した。
   本文は書き換えず訂正を追記した（不可逆な削除をしない）

### AI 側の誤りを 1 件記録する

判断キットへの初回回答は **Q1=B（6.18.0 へ昇格）と Q2=A（C-3 を critical）で両立しなかった**。
Q2=A のままでは規則「部分実装では上げない」に反し、さらに検査 9 が FAIL する
（`VERSION=6.18.0` で v6.19.0 に `実装済み` を書くと
「`実装済み` を名乗る spec の版 v6.19.0 が VERSION=6.18.0 を超えている」— 実測で確認）。
AI が矛盾を指摘し、人間が Q2 を B へ改めて整合させた。

**キット側の設計不備でもある**: Q1 の選択肢 B の説明文が「C-3 を standard 相当と見て」と
前提を内包していたのに、Q2 を独立した問いとして並べたため、両者が矛盾する組を選べた。
**依存のある 2 問を独立に見せた**のが原因で、次回の判断キットでは依存関係を明示する
（あるいは 1 問に畳む）。

### 昇格の実行（規則 §昇格の実行 の 4 点・同一 commit）

1. `VERSION` 6.17.0 → 6.18.0 ✔
2. `GRAPH.yml` の `version:` → `"6.18.0"` ✔
3. `upgrade-spec-v6.18.0.md` の状態行 → `実装済み（PR #261 / #262 / #263 / #264 / #268 / #270、VERSION 6.18.0）` ✔
4. `history/CHANGELOG.md` に「この版を持つと何が違うか」の節 ✔

**昇格 PR を 1 本立て、その merge を昇格時点とする**（人間判定 Q3=A）。規則の
「merge 時に上げる。遡及しない」を満たすため、F の merge とは別 commit・別 PR にした。
v6.11.0 → 6.15.0 の昇格で採られた「一度きりの負債として UPDATE.md に記載」の形は取らない。

### 持ち越し（人間判定）

- **D-3**（暴走案件の外部呼び出し元）: **打ち切り**。Cloudflare 側からは原理的に辿れず、
  書込は 54,339/日 → 345/日 へ落ちている。呼び出し元は今もエラーを受け続けている
- **D-4**（Stack 12 の `runtime_profile`）: **次の実案件の §7.4 自己検証で実測**
- **KV 書込の検出死角**: **埋めない**。`minna-no-ai-bbs-state` が 288/日（枠の 28.8%・
  `288 = 1440÷5` の形）を定常消費しているが、`flat_daily_writes` はリソース別 `rows_written` に
  しか掛かっていない。「罠は実害からのみ増やす」に従い死角の存在だけを記録する

---

## auto-merge メタ承認機構 — AI 判定漏れ記録 #2（2026-09-11）

`auto-merge-boundary.md` §メタ承認機構 の定める「AI 判定漏れ監視」の記録。
**記録 #1（2026-08-26）と同じ機構・違う破れ方**である。#1 は「付けても間に合わなかった」、
本件は **「そもそも付けていない」「人間に確認していない」**。PR #197 の機構修正は本件に効かない。

- **対象**: PR #280（`feat(quota): 共有枠の観測機構を新設 — v6.19.0 F2`）
- **区分**: AI 判定漏れ 1 件（opt-in 領域の未検知 + §設計原則 3 の未実行）
- **人間判定**: 2026-09-11、ユーザー。**D-2 を追認**（`crosscut-quota-observer` の配置で確定）。
  F3（同じく opt-in 領域）は **stop ラベル無しで auto-merge 走行を許可**

### 事実

| 項目 | 実測 |
|---|---|
| PR #280 作成 | 2026-09-11T11:19:34Z |
| PR #280 merge | 2026-09-11T11:20:15Z（**41 秒後**） |
| stop ラベル | **無し**（`human-review-needed` 未付与） |
| PR 作成前の人間確認 | **無し** |

**#280 は opt-in 領域に該当していた。** `auto-merge-boundary.md` §opt-in 領域 の
「**harness-verifier 自己改修**（`harness-verifier/**`）」に `harness-verifier/glossary.yml` の
改修が該当する。AI（本セッション）はこの該当を検知せず、§設計原則 3 が要求する
「対話中に必ず stop ラベル付与確認を発話する」を実行しないまま PR を作成した。

**PR 本文には「`human-review-needed`」と自ら書いていた。** 本文 §人間の判断が要る点 に
「D-2 は人間専管。追認前に merge すると記録と実態が食い違う。`human-review-needed`」と記載しながら、
ラベルは付けていない。**宣言と実行が同一 PR 内で乖離している。**

### #1 との差分 — 機構修正が効かない理由

記録 #1 の是正（PR #197）は「ループ内の毎反復検査 + merge 直前の再判定」であり、
**ラベルが付いていれば止まる**ようにした。本件はラベルが付いていないため、その修正は起動しない。

`auto-merge.yml:408` の `FINAL_STOP` 再判定は正しく動いている。破れたのは機構ではなく
**機構の入力を与える側（AI）**である。

### 前提の誤り（本セッションが一度誤って整理した内容）

本セッションは当初この現象を「merge のタイミングを確認せずに push した手順ミス」と整理し、
さらに PR #281 本文で「ラベル付与の前に merge が完了したため」「このリポジトリではラベルによる
merge ゲートが機能しない」と書いた。**後者は誤りである。**

- ゲートは機能する。**入力が与えられなかった**だけである
- 41 秒という時間は「間に合わない」根拠にならない。§設計原則 3 は
  **PR 作成前**の人間確認を要求しており、作成後の付与を前提にしていない
- PR #278（F1）は `human-review-needed` を持つが、本セッションの付与は merge 後だった可能性が高い
  （merge 10:26:21Z に対し、同件の後続コミット `aa7067d` の author time が 10:26:51Z）。
  **#278 も同じ破れ方をしている**可能性があり、#1 のような機構起因とは限らない

### 是正（本セッション内）

- v6.19.0 F3 以降、**`.claude/skills/**` を 3 ファイル以上触る PR は作成前に人間へ発話確認する**。
  F3 は該当（4 ファイル）のため、PR 作成前に停止して確認した
- ラベル付与は **PR 作成の直後の 1 手**に固定する（別作業を挟まない）

### 人間判定へ渡す論点

- **#280 の事後追認**: D-2（観測 skill の prefix・命名・配置 = `crosscut-quota-observer`）は
  人間専管の判断点であり、追認を経ずに merge された。**この配置でよいかの追認が要る**
- **§設計原則 3 は AI の行動規範であって機械検査が無い**。記録 #1 の哲学者軸が述べた
  「規定されたゲートが誰の担当でもないため起動しない」の 6 件目にあたるかどうか。
  **本件で発話確認そのものは機能した**（F3 では PR 作成前に停止し、人間が判断した）。
  機能しなかったのは #280 の 1 件であり、原因は AI が opt-in 該当を検知しなかったこと
- 判定日: 2026-09-11
- 判定者: ユーザー（`toriaezu.hide@gmail.com`）
- 判断: **(1) D-2 を追認** — `crosscut-quota-observer`（`crosscut-` prefix / Level A / D4 /
  scripts・adapters を skill 配下に同梱）で確定。#280 の事後追認をもって記録と実態の乖離は解消する。
  **(2) F3 は stop ラベル無しで auto-merge 走行を許可** — opt-in 該当を承知の上での明示許可であり、
  §設計原則 3 が求める「人間への発話確認」は履行された（確認の結果が「走らせてよい」だった）。
  ラベルは「人間が止めたいとき」の手段であって、確認の成立条件ではない
  **(3) v6.19.0 の残り（F4・F5）は一括で許可**（2026-09-11 追記） — 同 spec の実装範囲内に限り、
  opt-in 該当でも PR 作成前の個別確認を省略し stop ラベル無しで走らせる。
  **spec 外の改修が必要になった時点で再度確認する**。§設計原則 3 の「発話確認」は
  範囲を明示した包括許可でも成立する（毎 PR の逐次確認を要求する条文ではない）
- 補足: 本記録は `auto-merge-boundary.md` 自体を書き換えていない（L-FROZEN-META）。
  #280 の 1 件を判定漏れとして計上し、機構の再評価は既定のスケジュールに渡す

---

## auto-merge メタ承認機構 — AI 判定漏れ記録 #1（2026-08-26）

`auto-merge-boundary.md` §メタ承認機構 の定める **「AI 判定漏れ監視」の記録**。同節は
「AI が opt-out 判定した PR で後から人間が `human-review-needed` を付与した場合、
その PR を『AI 判定漏れ』として記録（手動運用）」と定め、集計先を本ファイルとする。
本節は当該規定に従った append であり、境界 SPEC 自体（L-FROZEN-META）は書き換えていない。
...
AI（本セッション）が opt-in 領域（workflow 自身）に該当する PR を **ラベルなしで作成**し、
直後に `human-review-needed` を付与した。しかし auto-merge は停止せず squash merge した。

| 時刻 (UTC) | 出来事 |
|---|---|
| 03:45:20 | 条件 1（stop ラベル）を評価 — この時点でラベル不在 |
| 03:45:21 | 条件 3.5 のポーリング開始（最大 `POLL_MAX_WAIT=1500` 秒） |
| 03:45:26 | `human-review-needed` 付与 |
| 03:45:52 | 「全条件 pass。squash merge します」 |
| 03:45:56 | マージ完了 |

### 二重の欠落

本件は「AI がラベルを付け忘れた」だけではない。**付けても間に合わない**構造だった。

1. 条件 1-3 がポーリングループ前で 1 回だけ評価され、merge 直前に再判定されない
2. `concurrency.cancel-in-progress: false` により、ラベル付与が起こす `labeled` event の run は
   in-flight run の後ろに queue される（実測: labeled run の job `created_at` 03:45:58 は
   in-flight run の job 完了 03:45:57 の 1 秒後で、マージ 03:45:56 に間に合っていない）

ゆえに **外部 event は in-flight run を止められない**。

### 先行事例との関係

`delivery/ANALYSIS-role-boundary-2026-08-25.md` 観測 #4（PR #191 が opt-in 領域なのに
自動マージされた件）は当時「付け忘れ」と解釈された。本件の証拠は「付けても間に合わなかった」
可能性を示すが、**#191 の再解釈は行っていない**（当時の job ログでポーリング反復の有無と
ラベル付与時刻を突合すれば決定論的に判定できる。未実施・次サイクル申し送り）。

### 是正

PR #197 で機構側を修正（ループ内の毎反復検査 + merge 直前の再判定、fail-closed）。
ただし最終 `gh pr view` と `gh pr merge` の間に**数秒の TOCTOU は原理的に残る**。
窓は 25 分から数秒へ縮むが「塞いだ」わけではない。

### 2026-11-06 の再評価へ渡す論点

Council `council-2026-08-26T04:04:34Z-amrace` の哲学者軸（少数意見）:

> opt-in 領域とは「人間が能動的に GO と言うまで止まる」領域であり、それを blacklist ラベル
> （誰も止めなければ進む）で表現した時点で範疇錯誤が確定していた。#191 と #196 は同一の
> 錯誤の二つの現れである。

同軸はさらに「**規定されたゲートが誰の担当でもないため起動しない**」を `ANALYSIS-role-boundary`
観測 #1〜#4 と同型の 5 件目として提起した。本記録はその指摘に対する応答であり、
判定は人間が行った（AI は記録のみ）。

---

## v6.8.0（minor 昇格、CLAUDE.md 統括者モデル + ローカル自律実行ファースト）

- 判定日: 2026-08-05
- AI 能力バージョン: claude-opus-5
- 改修主体: layer0-spec-architect 対話セッション（L0 メタハーネス開発、kakuman メタ診断 2026-08-04 起点）→ 同セッション内 L1 実装
- minor 昇格の根拠: 後方互換維持（既存プロジェクトの CLAUDE.md は never_touch・移行任意。参照リンクの削除・改名なし。
  純化 RL は v0.1.0 の枠組みを保持したまま §7〜§9 を追加。dev-env-spec §CLAUDE.md は見出しリンク単位で保持し内容改訂）
- モード判定: M2 標準（S=1 ファイル 6 件・仕様明確 / U=1 対話で解消済み / R=2 DH は配布物 = 利用者プロジェクトへ波及、
  R≥2 で M2 強制。L2 閾値未達）
- 儀式記録: レベル 2（SUMMARY + 関連 INTENT 読込。検出 2 件 — ①新欲求は既存 200/300 行規定の「形骸化の是正」であり
