...
## v7.0.0：現行モデル世代向けメタハーネス刷新 — 認知足場の剥離・環境足場の強化 (in progress: Phase A PR-2)

利用者の「既出の情報も含めて DH の刷新を設計する。現行最新モデルに合わせた対話設計・自律実行メタハーネスの自己開発を開始する」
（2026-09-13、L0 メタハーネス開発）を起点とする major 候補。前段の診断（ハーネスの半減期 / メタハーネス再構築診断・PR #287）を入力に、
L0 儀式レベル 3 → 人間決定 4 件 → Council `v7phsa`（案C）→ L1 実装（PR-1）の順で進行中。

### 設計意図の核

- **線は「認知 / 環境」**: モデルに吸収されたのは認知を教える足場（多ペルソナ合議・役割分割・段階思考・自己批評）で、
  環境を区切る足場（決定論検査・文脈隔離・権限段階・skill 絞り・失効可能な記憶）は残っている。足場差だけで同一モデルが
  最大 28pt 動く以上、全剥離は誤り。剥がれる足場と残る足場を分ける
- **2 段構成**: Phase A（L-FULL・今）は憲法 3 文書に触れない。Phase B（H1・2026-11-06 ゲート後・人間起票）で philosophy を
  不変条 / 運用条に分け、第 7 条 4 役割を F8 の実測で決める。`VERSION` 7.0.0 は Phase B
- **削る前に測る（I-5）**: 部品を削る判断は基準凍結 → 固定ケース → 劣化した分だけ戻す。Council 自身の ablation（F8）にも、
  自らの description 圧縮（F3）にも同じ規律を課す（自己適用の非対称を作らない — 哲学者軸）
- **失効は宣言ではなく購読からの離脱で効く（F6・PR-2）**: 「廃止」と書いた規範が購読に残れば記憶は害になる（append-only 0.210 <
  記憶なし 0.309 < 明示失効 0.950）。失効を機能単位（INTENT）から規範単位（罠 / RL / 叡智節）へ下ろし、`status` = `revoked` →
  norm-scan 列挙 → 儀式で人間が COLD 移送を承認 → 次サイクル排泄、の経路を通す。判定は機械に持たせない（第 8 条）
- **配線済み・中身なしを埋める**: PreCompact 観測（注入不可）・`model_generation`（永久不発火）のように、機構はあるが実行主体や
  payload が無い箇所を優先する。新機構の発明より安く、効果は実測で出る（発火 0 → 3）
- **パージは品質根拠（I-2）**: トークン削減を目的にしない。請求の大半はキャッシュ traffic で、削って増える実測がある

### 廃止

- **廃止**: 2026-09-13 — `crosscut-verifier-philosophy`（v5.0.0 placeholder のまま 6 版後送・CTL-1 では起動条件未達・利用率 0）。
  deprecation-protocol: 利用率 0 / 代替は Council 経由・人間判定 / 依存なし（参照 12 箇所を廃止注記）。思想検証は Phase B の
  philosophy 分割後に再設計する（D-2・人間一言承認「すべて実行」）

### 温存（Phase B / 観測駆動）

- 第 7 条 4 役割の縮退可否（F8 の材料: (a) 一致 0.976 だが rejected 0 / (b-snap) 一致 0.80・第 3 の道 2 件を出せず・コスト 1/4〜1/6）
- 第 8 条 minority C（4 段階拡張）との整合は Phase B で同時に扱う
- `/context` による一覧枠の実消費（等級 C 由来の 4.5〜6.5 倍）は人間が 1 回実測してから F3 の投資規模を決める
...
- **設計図は既にある、足すのは実行基盤だけ**: DH はオーケストレーションの設計図（Council フェーズ
  プロトコル・review OC+workers・反証 fan-out・L2 雛形）を規範として持っていた。外部ツールに合わせて
  設計図を書き換えるのではなく、設計図をそのまま写せる実行基盤（Workflow = 決定論 / 議論型協調層 = 抽象契約）を
  選ぶ。「減らす・時限化」原則との整合は**置換**という位置づけで担保
- **記録 teeth の構造化**: 軸監査 WARN の根因（options 正規化・dimension 記録漏れ・confidence 帯非強制）を
  スクリプトの schema 強制と §8 ブロック生成で封じる。手順書依存の空文化（v6.1.0 CTL 分断と同型）を
  規律ではなく構造で防ぐ
- **実行基盤は判定を持たない（I-1）**: judgment は Council・決定は人間/合意プロセスのまま。実行の機械化と
  判定の委譲を混同しない（escalation-matrix §3 に明文化）
- **抽象契約 + 時限付き随時層（F6）**: Research Preview（Agent Teams）を規範が直接参照すると破壊的変更の
  たびに規範改訂が要る。規範は抽象契約 1 文のみ、固有情報は時限メタデータ付き templates/experimental/ に
  隔離し、GA 化 or 2026-11-30 で再評価（時限化機構の自己適用・期限超過 WARN teeth）
- **degrade は従来動作への回帰（C-2）**: 実行基盤の不在・失敗は機能停止ではなく従来 subagent 経路への
  回帰。DH は Workflow tool なしでも完全動作する（「GitHub 無しでも完全動作」と同型の独立性原則）
- **器外の観測点（C-3、哲学者条件）**: Council 自身の器を Council 判定で GO する自己言及構造への対処として、
  機械化が判定の質（多様性・独立性）に与える影響を軸監査 + 振り返り儀式の定点観測に接続。schema 強制の影
  （schema 外の異見の沈黙）には persona 出力の notes 自由記述欄で弁を残す

### 開放問題（観測駆動で次サイクル以降）

- F2 のフィールド単位 schema 強制（現状は JSON 構造化のみ。output-format 正典との整合検査は運用データ待ち）
- Workflow スクリプト決定論部の単体テスト規格（開発者軸 concern。scripts/test-* の慣行に載せるか）
- 挙動同一性の長期回帰（受け入れ基準は最初の 3 発動のみ。以後は軸監査が代替観測）
- 議論型協調層の実運用評価（degrade 常態化していないか = 名目化の検出、F6 再評価時）
...
- **配線表であって新規範ではない**: escalation-matrix は philosophy 第6条（H/C カテゴリ）・
  第9条（委譲境界）・献上哲学・Council 発動基準という既存規範を「ポジション×段階 → 判定先」の
  一表に固定するもの。新しい判定カテゴリ・新しい権限を発明しない（認識の負債を増やさない）
- **承認スコープの構造化**: 利用者の一文依頼は「着手の承認」として有効だが、規範改変の
  「内容確定の判定」は別途促す（実装前 Council 諮問 + 献上時人間判定）。依頼文言の解釈を
  実行主体の裁量から外すのが f9b2c4 是正 (2) の実装形
- **矛盾は情報**: AI 判定矛盾（自己検証 vs 独立検証等）を一律 FAIL で握り潰さず、どの観測が
  割れたかを Council で構造化して緩和する。jc < 0.5 で人間へ、Council 自体が当事者なら
  直接人間へ（再帰遮断 — 第6条 憲法の自己改訂禁止と同型）。反証成立は矛盾ではなく FAIL 確定
  （falsification-protocol との優先順位を明文化）
- **自己適用**: 本マトリクス自体の改変が「規範文書改変」行の対象。本 v6.10.0 実装は f9b2c4 の
  合意プロセス内（escalate_to_human → 人間決定の実装）として実施され、判定の促しを経ている

### 温存（観測駆動で継続判断）

- f9b2c4 推奨 (1) の auto-merge パスベース機械ガード: 判定促し（規範層）を先行させ、
  機械ガード（決定論層）の追加是非は促しの遵守実績を観測してから判断
- f9b2c4 推奨 (3) の反証記録実効性観測: 次サイクルの振り返り儀式（F1-F3）に接続
...
DH はコンテキスト隔離（ループ分離）で検証の独立性を担保しているが、隔離された reviewer の検査は
**確証（SPEC⇔成果物の一致証拠集め）に閉じており、反証（壊れる入力・状態の探索）を明示的に課していない**。

### 設計意図の核

- **確証と反証は向きが逆の別検査**: 隔離だけでは「テストが正しいものを見ているか」（Test Oracle の
  妥当性）に届かない。独立性には「ループが別」と「観測の向きが別」の 2 層があり、後者を機構化した
- **反証 3 類型**（A 挙動 / B テスト / C Oracle）は罠 C2（自己言及の罠）の E2E 限定隔離（5.5.2、
  v5.23.0）を全テスト層へ一般化したもの。新原理の発明ではなく既存検査の姿勢の明示化
- **リスクベース打ち切り**: 反証は尽きないため、critical 必須（各 1 件以上・上限目安 3 件）・
  standard 任意で打ち切る。ただし省略は隠さず「この PASS が保証しない範囲」必須欄で外部化する
  （そのGreenは何を保証しているか、を毎回の VERIFICATION.md に強制する）
- **認識論的規律**: 反証不成立 ≠ 正しさの証明。PASS の意味を「確証全通過 ∧ 試行した反証が全て
  不成立」という有限の主張に再定義した
- **既存規約との衝突回避**: B 類型（ミューテーション）は原状復帰確認必須、archeo preserve 領域
  （`no_modification`）では実行禁止（静的検出のみ）。agent 本体のプロジェクト不変性は維持
  （反証対象の導出元は SPEC / DONT / sensors の記述のみ）

### 残された開放問題（観測駆動で次サイクル以降）

- 検証者バイアスの同型性: 反証を課しても reviewer は実装者と同一基盤モデルであり、同じ盲点を
  共有しうる（ループ分離 ≠ バイアス分離）。Council のペルソナ分割は部分的応答だが根本解決ではない
- 反証試行の実効性測定: 「反証記録」が形式的な埋め草に堕ちていないかの監査（COUNCIL-LOG 実測と
  同型の事後測定）は運用データ蓄積後に検討
...
- **純化 RL v0.1.0（Council `claude-md-purity`、7/23 採択）を正本として拡張する**（新ドクトリンを並立させない
  = 二重定義回避の自己適用）。v0.2.0 で追加: §7 ルーティング表型索引 / §8 配置 drift 計算的センサー / §9 検証のローカル性
- **ルーティング表型は罠索引型とのトレードオフ**: per-領域 1 行に粗くできるのは「ゲート強制 3 段
  （gate ID 記述 → DELIVERY 通過ゲート記録 → ルーティング突合センサー）」があるときのみ。強制なしの粗粒度化は
  到達可能性（§3）違反。純化 RL v0.1.0 が per-規範 96 行を許容したのは強制機構がなかったから、という関係を明文化
...
- **ローカル自律実行ファースト原則（2026-08-05 ユーザー宣言・2 段階で研磨）**: ① AI エージェントの自律実行は
  ...
  ローカル）。**CI にしか存在しない検査を作らない**。「CI ミラー」概念は廃棄（検査が CI 側にもあるという前提の残滓）
...
- `templates/github-workflows/`（auto-merge / gemini-review 等の独立検査 workflow 群）の位置づけ再定義。
  ローカル自律実行ファースト原則と緊張関係にあるが、autonomous-drive 全体の再設計を伴うため観測駆動で次サイクルへ
  （先行事例: kakuman の Actions 無効化 + Vercel ゲート自動マージ + ローカル独立レビュー）
- kakuman `rls-drift.yml`（日次 CI センサー）のローカル変種 — 利用者プロジェクト側の課題として指摘のみ
- 純化 RL §6 の条件付き（常駐購読モデル前提）は不変。常駐/随時の非対称が消えたら再諮問
...
**(a) 専用サイクルを作らない**: DH には完成度の高い情報代謝サイクル（北極星=購読量上限 / HOT・WARM・COLD /
Council ゲート / 8 不変条件 / cursor 増分 / source pointer）が既にある。E2E のために別サイクルを新設するのは
重複であり構造同形を壊す。∴ **E2E run 履歴を既存代謝の新しい episodic ソースとして流す**のが正解。

**(b) E2E は project のもの（利用者明示・Council mtbl01 と一致）**: run 履歴・flaky・artifact は「このアプリの
この画面」に閉じた事実＝ project 還元（D1-D3）。正本は利用者プロジェクトの REGIME.md `## 情報代謝設定`。
**DH 本体は対面アプリを持たないため E2E 代謝を稼働しない（定義のみ・dog-food 対象外）**。一般代謝は dog-food
するが E2E はしない、という非対称をはっきり持つ。DH に入るのは「機構の正本(D4)」と「project への scaffold」のみ。

**(c) 二相分離が COLD/HOT に直結**: v5.23.0 の相 A（in-loop 知覚器・使い捨て）/ 相 B（耐久資産）が、そのまま
COLD（artifact・生 run ログ）/ HOT（flaky 罠・安定 journey RL・oracle 結晶）に対応する。設計が一貫する。

**(d) 北極星（購読量上限）を E2E で守る最大の急所**: E2E は毎 CI 実行で大量データを生む。これを素朴に貯めると
購読量が線形膨張する（代謝天井）。∴ **artifact は COLD 直行で既定ロードせず**、蒸留された flaky 罠だけが HOT に
乗る。履歴（disk）は無限に太ってよいが、購読量は密度↑（量↑でない）に保つ。E2E はこの原則の最も厳しい試金石。

**(e) 8 不変条件の E2E 具体化**: #1 単発 flaky を罠にしない（council_gate 反復ゲート）/ #6 罠は生 run ログ
（COLD lossless 原本）から蒸留・E2E-LOG の lossy 要約からは結晶化しない / #8 flaky 罠は反-発火条件必須
（誤発火が debug 困難な flaky で特に重要）。

### サイクルと teeth

5 フェーズ（記録→蓄積→代謝→還流→結晶+排泄）のうち④還流だけが新規 teeth（flaky 率超過→auto-merge 停止 /
circuit-breaker / Quarantine→設計層）。v5.23.0「接続地図のみ」の本体化だが、**強制連動は opt-in/段階**に留める
（実 E2E 運用データで閾値を較正してから＝metabolism 自身の Dry-run 精神・auto-merge-boundary の慎重さ）。

### 温存

- **mutation メタテスト(C4)**: 重いため温存。
- **④teeth の強制化**: auto-merge 自動停止の強制連動は次 PR で運用データ較正後に再判定。
...
**(a) E2E = AI の知覚器官**: 人間 dev は実行中アプリを目で見て手で触るが AI にはその経路が無い。
E2E + artifact + Vision が AI の唯一の現実接地チャネル。ゆえに **AI は「見ると宣言したものしか見えない」**、
AI の知覚は「何を検証するか」宣言の精度に律速される。これが後述 C5 を哲学の本丸にする。

**(b) 二相分離（一次概念）**: 相 A（in-loop 知覚器・自己修正用・使い捨て可）/ 相 B（SPEC 由来の耐久資産・
回帰契約）。人間の「テスト=記憶代替」に対し、DH では次エージェントが SPEC+history から文脈を再構成するため
耐久テストの役割は「SPEC とコードの実行可能契約」＝ drift の runtime 補完へシフトする。

**(c) C5 テスト oracle 言語化（人間が最重要と判断）**: 人間の暗黙の関心（何を気にするか/なぜ作るか/
暗黙前提）を言語化すること = AI の知覚宣言を拡張すること = AI の知覚野を広げること。第4条（人間=意図共有/
AI=構造化）・第1条（認識ズレゼロ化ループ）をテスト設計に適用したもの。SKILL.md §2.6 に昇格。

**(d) AI テスト精度問題と自己言及の罠**: AI は低信号テストを過剰生成しうる/間違ったものを検証しうる。
根に「実装した AI が・テストを書き・自分で通ったと判定する」自己言及の罠（第3条 独立性で隔離）。
対策 C1（SPEC 由来母集団）/ C2（生成と判定の隔離）/ C3（本数天井）/ C5（L0 で人間と大枠合意）。

**(e) browser provenance pinning**: 「どの browser が開くか」のばらつき（CC コンテナ vs ローカル実 Chrome
vs CI）は AI の知覚器官そのもののばらつき＝ flaky 源・観測性統一（第6条）の侵食。「借りない・固定する・
記録する」で統一。カノニカルエンジン = 同梱 chromium 単一既定、webkit/firefox は SPEC 宣言時のみ opt-in。

### 温存（minority opinion・将来 PR 再判定）

- **テスト情報代謝**: 相 A を cycle 後に COLD 退避/削除し相 B を結晶化する「テスト版 HOT/WARM/COLD」。
  情報代謝の射程にテストを含める新しい一歩だが、本導入自体が試験的なため実 E2E 運用データが貯まるまで温存。
