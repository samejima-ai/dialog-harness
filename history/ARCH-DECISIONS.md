...
| 影響 | `layer0-archeo-architect/references/handoff-to-evaluator.md` のステータスヘッダ / ロードマップ表 / 先行宣言 4 セクション 3 箇所のみ改修。SK 本文の機能変更ゼロ、既存 references の本文（追記のみ）、crosscut-* / templates/ / harness-verifier/ は機能不変。`refactor-intent-map.md` の I/O 契約不変。先行宣言 2/3 との整合性ガード（schema priority）を新設、今後の先行宣言 2 更新時に整合確認義務発生 |
