...
誰も読まない状態が終わる）。**(2) 供給元アカウントの共有枠を機構が観測できる**
（1 案件の暴走が同一アカウントの全案件を止める事故に、人間の手を介さない検出経路がついた）。

6.18.0 は **v6.18.0 と v6.19.0 の両方の内容を含む**。spec 番号が release 番号を追い越した形であり、
`dev-env-spec.md` §spec 番号は起草時の予約 が想定する状態そのものである（先例: v6.12.0 は
VERSION 6.15.0 で `実装済み` を名乗る）。

### v6.18.0 由来（時限の主体化）

- **C-1** 時限メタデータを読む主体 — `scripts/norm-scan.py`（PR #261。`drafted_as: v6.13.0 F5`）
- **B-1** 点検にリズムを与える — 期限条項付きの点検（PR #262）
- **B-3** 「静かな失敗」を辞書に載せる — `glossary.yml`（PR #261）
- **C-2** 検査 8 の分割 — 欠落の型で切る（PR #263 / #264）
- **C-4** 検査が静かに無効化する経路を塞ぐ（PR #270）。**入力を消すだけで検査が黙って skip し、
  同じ緑が返る**経路を実測して塞いだ
- **F0** 版番号の自己適用 — 存在しない版を名乗って ship していたことの是正（PR #268）

### v6.19.0 由来（供給元の枠）

- **F1** Supabase 既定性の降格（PR #278）。**既定が到達不能な供給元を指していた**
  （2 アクティブプロジェクト上限で既に満杯）
- **F2** `crosscut-quota-observer` 新設（PR #280）。起点は実測 —
  **1 プロジェクトが D1 日次書込枠の 53% を単体で消費**しており、超過すれば同一アカウントの
  全 D1 クエリが失敗する構造だった。**機構は何も知らせていなかった**
- **F3** Cloudflare プレイブック（PR #282）。罠 C1〜C11。**数値表を持たず観測記録を指す**構成
- **F4** Stack 12 + 観測の配線（PR #283）。`GRAPH.yml` edges 41 → 43。
  F3 の時点で edge がゼロ＝**在るのに誰も呼ばない機構**だった
- **F5** データ層の出し分けを機械判定に（PR #284）。**Hyperdrive は共有枠から逃げる手段ではない**

### 昇格の判定（人間専管）

判断キット `delivery/DECISION-KIT-version-promotion-2026-09-11.html`（PR #285）の 5 問に対する
人間回答で確定した。**キットの初回回答は Q1=B と Q2=A で両立しなかった** — Q2=A（C-3 を critical）
のまま Q1=B（6.18.0 へ昇格）を採ると、規則「部分実装では上げない」に反し、さらに検査 9 が
FAIL する（実測で確認）。指摘の上で **Q2 を B（C-3 を `priority: standard`）へ改めて整合させた**。

- **C-3 は未着地のまま昇格した**。規則の「standard / cosmetic の未実装は昇格を妨げない」に従う
- **v6.18.0 F0 §是正 step 2 の前提が覆った**。同項は「C-3 の実装完了時に 6.18.0 へ昇格させる」と
  書いていたが、C-3 を standard と判定したため完了を待たずに昇格した。
  **本文は書き換えず訂正を追記した**（C-1 の `drafted_as` 追記と同じ扱い）
- **v6.19.0 は `実装済み` を名乗らない**。自版 6.19.0 > `VERSION` 6.18.0 であり、検査 9 の
  「実装済みを名乗る版 ≤ VERSION」に反する。**実測で FAIL を確認した上で**状態行を
  `実装中（F1〜F5 済 / 内容は VERSION 6.18.0 に含まれる・6.19.0 への昇格は未実施）` とした

### 同時に持ち越したもの

- **D-3**（暴走案件の外部呼び出し元）は**打ち切り**（人間判定）。Cloudflare 側の書込は
  54,339/日 → 345/日（削除前の残り）へ落ちたが、呼び出し元は外部にあり Cloudflare 側から辿れない
- **D-4**（Stack 12 の `runtime_profile`）は**次の実案件の §7.4 自己検証で実測**（人間判定）。
  `local-reproducible` は仮置きである旨を Stack 12 節に明記済み
- **KV 書込の検出死角は埋めない**（人間判定）。実測で `minna-no-ai-bbs-state` が
  288/日（枠の 28.8%・`288 = 1440÷5` の形）を定常消費しているが、`flat_daily_writes` は
  リソース別 `rows_written` にしか掛かっていない。**罠は実害からのみ増やす**の規律に従い、
  死角の存在だけを記録する。同型ツールをあと 3 つ作ると枠が埋まる点は量産判断の材料として残す

## Hyperdrive は共有枠から逃げる手段ではない — データ層の出し分け（v6.19.0 F5、2026-09-11）

**この版を持つと何が違うか**: Cloudflare 案件で **D1 か Hyperdrive + 外部 Postgres かが
3 条件の機械判定で決まる**。「消失許容度はどれくらいですか」という主観語の質問が消え、
判定が AI 推論に依存しなくなる。v6.19.0 の最後の F。

`cloudflare-workers-dev.md` に §データ層の出し分け を新設し、`dialog-questions.md` S1 と
`scaffold-checklist.md` §Stack 12 に配線した。

| # | 条件（1 つでも該当 → Hyperdrive + PG） |
|---|---|
| 1 | Time Travel の保持期間を超える復旧要求があるか |
| 2 | 単一 DB が容量上限を超える見込みか |
| 3 | DB 間 JOIN または Worker 外からの接続が要るか |

**新規質問は 1 問だけ増やした**（条件 1 = 「**何日前まで戻せれば足りますか**」）。条件 2・3 は
S1 以降で既に判明している情報から yes/no が決まるため質問を要しない。

### 閾値の数値を分岐表に焼き込まなかった

spec は条件 1 に「無料 7 日」、条件 2 に「500 MB」と具体値を書いていたが、いずれも供給元が単独で
書き換えられる値であり I-3 に触れる（F3 と同じ判断）。表には**条件の形**だけを置き、
**閾値は判定時に観測記録から取る**構成にした。

> **規範が持つのは条件の形、観測が持つのは閾値の値。**

「Time Travel 7 日で足りるか」と訊くと 7 が変わった瞬間に質問が誤りになる。
「何日前まで戻せれば足りるか」なら質問は腐らず、比較対象だけが観測から来る。

### spec が言っていなかった重要な事実 3 件

一次情報の確認で判明した。いずれも判定を変えうる。

- **Hyperdrive は共有枠から逃げる手段ではない**（最重要）。無料 10 万クエリ/日は
  **アカウント単位で共有**され、超過すれば失敗する。`SELECT` だけでなく
  `INSERT` / `UPDATE` / `DELETE` / `CREATE` / `ALTER` / `DROP` も 1 クエリとして数える。
  **spec は「Hyperdrive は無料プランでも利用可」とだけ書いており、共有枠からの出口と読める
  余地があった**。読めてしまうと F2 の観測が Hyperdrive 案件を素通りする。加えて外部 PG 側の枠
  （Supabase の 2 アクティブプロジェクト上限など）が**二重に効く**
- **ローカル再現性が D1 と違う**。D1 は `.wrangler/state` の SQLite で完全オフラインだが、
  Hyperdrive は `localConnectionString` で**到達可能な PG が必要**で、さらに
  **ローカルではクエリキャッシュが効かない**。ゆえに PG プロファイルの `runtime_profile` は
  一律に決まらず「ローカル PG を立てるか」に依存する
- **Hyperdrive 固有の罠 4 件（H1〜H4）**。特に H1 = **キャッシュ可否の判定が文字列パターンマッチ**で、
  `NOW()` 等の関数名が **SQL コメント内にあるだけ**でもキャッシュ不可になる

Hyperdrive の枠の値は観測記録（`adapters/cloudflare.md`）に 1 行として追加した。**閾値の正本は
引き続き 1 箇所**であり、腐敗面は増えていない。

### `実装済み` を名乗らなかった（新設 D-6）

F1〜F5 はすべて master に入ったが、**状態行は `実装中（F1〜F5 済 / VERSION 昇格は人間判定待ち）`
に留めた**。`dev-env-spec.md` §状態行の値域 は `実装済み` を「その版の critical がすべて master に
入り、**VERSION も昇格した**」状態と定めており、VERSION 昇格は人間専管である
（過去の昇格記録はすべて判定日・判定者・昇格根拠を人間名義で持つ）。

**最初は `実装完了（F1〜F5 すべて着地）` と書いて検査 9 に FAIL した**（値域外）。
検査が「AI が状態行だけ先に進めて宣言が実態を追い越す」のを止めた形で、
v6.18.0 C-4 が塞いだ乖離のちょうど逆向きの事故にあたる。判断点 **D-6** を新設して人間に渡した
（6.17.0 → 6.19.0 の飛び越しになるか v6.18.0 を経由するかも含めて判定が要る）。

### v6.19.0 の未解決（3 件）

| # | 内容 |
|---|---|
| D-3 | 暴走案件の外部呼び出し元が**未特定**。cron が空なのに日次 5.4 万行の書込があった。罠 C10 として構造だけ記録 |
| D-4 | Stack 12 の `runtime_profile` が**未実測**（`local-reproducible` は仮置き。明記済み） |
| D-6 | `VERSION` 昇格（人間専管・新設） |
...
**設計上の罠**が L0 対話から引ける。Docker 不要・`wrangler dev` は既定でローカル（本番と同じ
`workerd` が動く）ため、`supabase-local-dev.md` より導入コストが低い。

`.claude/skills/layer0-spec-architect/references/cloudflare-workers-dev.md` を新設し、
SKILL.md §6 の供給元別プレイブック表・§参照ドキュメント、`dialog-questions.md` S1 に配線した。
**既定にはしていない**（供給元選択は L0 対話で人間が行う。既定を置くのは F4 の Stack 12）。

**spec の指示から意図的に逸脱した 1 点**: spec F3 は無料枠の数値をプレイブック本文に書くよう
指示していたが、**従うと I-3 に違反する**ため採らなかった。F2 の `adapters/cloudflare.md` が
同じ数値を観測記録として既に持っており、再掲は腐敗面を 1 → 2 に増やす。これは F1 の独立検証 P3
が検出した違反と同型（「規範側は観測を参照せよだけを書く」と定めながら規範側 2 箇所に複製し
腐敗面を 1 → 3 にした）。**同じ失敗を二度やらない。**

- プレイブックは**数値表を持たない**。冒頭で観測記録（`adapters/cloudflare.md`）を正本として指す
- 「この表は腐る」と書く代わりに、**腐る表をそこに作らない**
- 罠カタログの正本もプレイブックに一本化。アダプタ側は観測に効く罠へ絞り、設計に効くものは
  プレイブックを指す（**同じ罠を二重に育てない**）

**罠カタログは C1〜C8 の予定が C1〜C11 になった**。実装時の一次情報確認で 3 件増えている。

- **C9**: Workers Cache を有効にすると、**通常無料の静的アセットリクエストも標準レートで課金される**。
  「速くするために入れたら課金が増えた」が起きる経路
- **C10**: cron が空でも書込は発生しうる（呼び出し元が Cloudflare の外にある場合）。D-3 の実測から
- **C11**: D1 は基盤更新で再起動しうる（`D1 DB reset because its code was updated.`）。
  **リトライ可能な一過性エラーが正常系に存在する**

**C6 は逆に数値を落とした**。spec の「書込 ~50 tps 程度で天井」は一次情報で確認できなかったため、
**未検証である旨とともに**記載し数値の引用を禁じた。出典を確認できない数値を規範に置かないのは
I-3 の趣旨そのもの。天井の存在（構造）は残し、値（観測）は空にした。

あわせて `--local` / `--remote` の明示を規律に入れた。`wrangler dev` は既定でローカルだが
`d1 execute` は**既定でリモート＝本番に当たる**。既定がサブコマンドごとに違うため、
「既定を覚える」ではなく「常に書く」に倒した。

**手順の是正**: 本 CHANGELOG 節と spec 状態行を**同じ PR に同梱**した。F1・F2 では PR 番号を
本文に書くために着地記録を後追いの別コミットにしたが、このリポジトリでは PR が作成直後
（#280 は 41 秒後）に merge されるため後追いは原理的に間に合わず、実際に 2 度とも落ちた。
着地の PR 番号は CHANGELOG の各節が持ち、spec 状態行は番号を持たない形に変えた。

## 1 案件の暴走が全案件を止める構造に、検出経路が無かった — 共有枠の観測機構（v6.19.0 F2 / PR #280、2026-09-11）

**この版を持つと何が違うか**: 供給元アカウントの**共有枠**（1 アカウントを複数プロジェクトが
食い合う枠）を機構が観測できるようになる。これまで DH に観測経路は無く、枠が埋まっていることは
人間が手で API を叩いたときにしか分からなかった。

**起点は実測**。2026-09-11 に 1 つのプロジェクトが Cloudflare D1 の日次書込枠の **53% を単体で
消費**していた。その枠はアカウント単位で共有され、2026-09-01 の仕様変更以降は超過するとアカウントの
**全 D1 クエリが失敗**する。無関係な既存プロジェクトも巻き込まれて止まる。**機構は何も知らせて
いなかった**。量産するほど、気づかないうちに枠が埋まる。

- `crosscut-quota-observer`（Level A / D4）を新設。`SKILL.md` + 検出器 + テスト + Cloudflare アダプタ
- 検出は `write_exceeds_read`（差分更新ではなく**全件書き直し**の signature）/ `flat_daily_writes`
  （利用者数に連動しない**定期バッチ**の形）/ 各種 `threshold_*`（共有枠の連続超過）。
  上 2 つが重なると全件同期バッチの確度が高く、これが 2026-09-11 の実測挙動そのもの
- **出力は 2 つ。検出 0 件でも消費率（headroom = あと何個作れるか）は必ず出る**。
  「検出 0 件」と「観測できていない」が同じ表示になると後者を前者と読み違えるため

**作らなかったものの記録**（spec の不変条件）:

- **I-6 判定を持たない**: 検出して数えるだけ。自動で昇格・停止・削除をしない（`judgment` は常に `null`）
- **I-2 共有枠は cross-project state**: per-project の宣言に書かない。枠は「プロジェクトの属性」ではなく
  「アカウントの状態」であり、per-project 軸に押し込むのはカテゴリエラー（Council `dt0911` 開発者軸）。
  当初案の `deploy_target` 新軸が退けられた理由もこれ
- **I-3 数値は観測記録であって規範ではない**: 枠の値はアダプタに観測日付つきで置き本体に焼き込まない。
  実際 2026-09-01 に D1 日次枠の**意味**が変わっている（超過しても通っていた → 全クエリ失敗）
- **I-1 供給元非依存**: 固有 API はアダプタ 1 ファイルに隔離。A-3 を完全には満たせない構造的制約
  （`crosscut-autonomous-drive` が GitHub に依存するのと同型）を、隔離で受け止めた

**独立検証で初版は FAIL**。4 件すべてが「配線されているが通電していない」型だった。

- **S1 配布経路の断絶**: 検出器をリポジトリ直下 `scripts/` に置いていたが同ディレクトリは**配布対象外**。
  skill だけが配られて実体が届かない構造だった
- **S2 出力の半分が未実装**: SPEC が定める 2 出力のうち消費率が無かった
- **S3 `threshold_rows_read` が完全な空振り**: 検出器を壊してもテストが落ちなかった
- **S4 `threshold_kv_writes` に供給経路なし**: `daily[].kv_writes` を埋める手順がどのアダプタにも無く、
  **永久に発火しない閾値**だった

テスト 24 → 47 項目。検証に「**検出器を自分で壊してテストが落ちるか確かめろ**」と指示し、
**32 変異のうち 5 件が空振り**であることを実測してケースを追加した。陽性ケースの中核は実測データ
（合成値ではない）、陰性は当該プロジェクト削除後の実ベースライン。「実リポで 0 件」は検出器が
空振りでも同じ表示になるため動作の証拠にならない（v6.17.0 の規律）。

**未解決として残したもの**: D-2（skill の prefix・命名・配置）は人間専管のため `human-review-needed`。
D-3（暴走プロジェクトの外部呼び出し元）は未特定 — cron schedules は空だったのに日次 5.4 万行の
書込があった ＝ 呼び出し元が Cloudflare の外にある。アダプタの罠 C7 に「**cron が空でも書込は
発生しうる**」として記録した。`GRAPH.yml` の edge は F4 で配線する。

## 既定が到達不能な供給元を指していた — Supabase 既定性の降格（v6.19.0 F1 / PR #278、2026-09-11）

**この版を持つと何が違うか**: 新規プロジェクトの L0 対話が、既に満杯の Supabase 無料枠へ暗黙に
誘導しなくなる。これまでは DH の規範 6 ファイルが Supabase を暗黙の既定として持ち、その既定が指す先は
**2 アクティブプロジェクト上限（ユーザー単位で全 org 合算のため org を増やしても回避不可）で既に満杯**
だった。既定に従うと新規案件は着手時点で詰まる。

- **設計変更ではなく運用事故の修正**。Council `dt0911` 必須条件 1 に従い、軸新設（F4）の可否から
  分離して先行実施した
- `supabase-local-dev.md` に **§適用範囲** を新設。「既存の Supabase プロジェクトを持つ案件にのみ適用」
  と限定し、無料枠を**観測日付つき**で記載。I-3 に従い「**この値は腐る**（供給元が単独で書き換えられる
  数値ゆえ規範ではなく観測記録）」前提を併記
- `SKILL.md` §6 本文から Supabase を除去して供給元別プレイブック表へ隔離、`dialog-questions.md` S1 の
  質問文から例示を削除、`scaffold-checklist.md` §Supabase 節にも適用範囲を明記
- **既存 2 枠は現状のまま使い続ける**（`paused` 化ではない）。Supabase は Postgres そのもので移行不要
  という価値が残るため `deprecation-protocol` は発動しない
- `schema-evolution.md` / `subphase-l02-domain.md` は hosted Postgres 一般の記述として正しいため触らない

**独立検証を 3 巡し 2 回 FAIL を受けた**。1 巡目は「3 ファイルの中では中立化できているが、委譲先で
打ち消されている」— 委譲先 `scaffold-checklist.md` の stack カタログに供給元選択が存在せず（Stack 1〜11 は
言語 / FW 軸）、同ファイル唯一の供給元記述「推奨バックエンド開発オプション ＝ Supabase」へ循環して戻る
構造だった。**降格したと報告したが降格していなかった**。

- **spec 自身の I-3 に実装が違反していた**（P3）: 「規範側は観測を参照せよだけを書く」と定めながら、
  無料枠の数値を規範側 2 箇所に複製し腐敗面を 1 → 3 に増やしていた。規範側から数値を落として一元化
- **是正が新たな事実誤認を持ち込んだ**（N1）: 「stack カタログはフロント／アプリ層のみ」と 5 箇所に
  書いたが、Stack 5〜9（FastAPI/Django/Express/Go/Rails）はバックエンドであり、同ファイル 3 行下の
  「**主に**フロント／アプリ層」および別箇所の「非 UI stack（FastAPI/…）」と正面衝突していた。
  結論（既定を置かない）は変わらないが根拠が崩れていた
- **計数記録の訂正**（P7）: spec の「67 箇所」は実体が `grep -ric` のマッチ**行数**で出現回数ではなく、
  ワイルドカードが `references/arc-patterns/` と `.claude/agents/` を取りこぼしていた。基準コミット
  `239d249` を明記し、取りこぼし 2 件を列挙して 67 + 2 = `.claude/` 全体 69 行の整合を提示。
  出現回数は大小区別 31 / 大小無視 96（3 巡目が 2 手法で確定、1 巡目の 95 は計測誤り）
- **注自身の自家撞着も是正**（S4）: 計数注が「原因は未特定」と書いた直後に「計数は環境差でもぶれる」と
  断定していた。I-3（観測記録の質）を説く注が未検証の因果を断定していては説得力を失う

**反証**: 新規案件で Supabase が既定になる経路を `.claude/` 8 ファイル + **`templates/` 33 ファイル**で
全走査。経路 A（scaffold-checklist 循環）/ B（曖昧回答時の提示）/ C（ロード条件）は全て閉、新規走査した
D〜J も既定を生成しない、`templates/` は Supabase / Vercel / Cloudflare いずれも 0 件。残る弱点は
「**唯一性による既定化**」（固有名を持つ供給元が依然 Supabase だけ）で、F4 で Stack 12 が入るまでの
正直な状態として spec に明記した。

## 検査が守っているものを消しても緑だった — 走査面の欠落を塞ぐ（v6.18.0 C-4 / PR #270、2026-09-07）

**この版を持つと何が違うか**: `harness-verifier --strict` の緑が「検査が走った上での緑」を意味するようになる。
これまでは入力を消すだけで検査が黙って skip し、同じ緑が返っていた。

- **実測した静かな無効化**: `GRAPH.yml` 削除で 6 検査が 0 件 exit 0 / `dev-env-spec.md` 削除・
  §バージョン履歴 の見出しレベル変更・改名・節削除でも exit 0 / `dh-upgrades/` 削除で 0 本走査 exit 0
- `_declaration_util.is_dh_core()` + `surface_missing()` を新設。**DH 本体では FAIL、配布先では
  METRIC で skip を可視化**（I-6 を壊さずに v6.13.0 I-4「検出器は黙って捨てない」を満たす）
- マーカーは `harness-verifier/checks/` 1 つ。**消すと検査自体が動かなくなる**ので、
  マーカーを消して欠落判定だけ黙らせる抜け道が原理的に作れない
- **検査 5b 新設**: draft 状態の spec について live 面が着地を名乗っていたら WARN。
  検査 5 は spec 自身の本文しか見ないため、着地の主張が実装側にあると素通りしていた
- **是正（I-1）**: v6.13.0 の状態行を実態へ（F5 は PR #261 で `norm-scan.py` として着地済みだった）。
  v6.18.0 C-1 に `drafted_as: v6.13.0 F5` を追記 — `dev-env-spec` §spec 番号は起草時の予約 が
  定めた `drafted_as` の**初適用**（リポジトリ全体で 0 件だった）
- **I-4 の担保**: 是正前 2 件 → 是正後 0 件。検査 5b の語は「着地 / 実装済」に絞り
  「予定 / 候補 / したい / すべき / する」で打ち消す（緩い版が「次サイクルで実装する予定」を
  拾う偽陽性をテストで実測し、締めた）
- 回帰テスト 21 件追加。走査面の欠落 3 経路 × DH 本体 / 配布先の両方、検査 5b の着地語 3 種と偽陽性 5 条件を実証
- **Copilot レビューの指摘で締めすぎを是正**: 初版は否定先読みと同時に「で実装」も削っており、
  `… を norm-scan.py で実装した` のような**真の着地主張を取り落として**いた。
  8 例で実測したところ先読みだけで偽陽性は 0 になり、語を削る必要は無かった。
  docstring は「着地/実装済/で実装」と書いたまま実装だけ狭めており、
  **宣言と実体の不一致を主題にした PR の中で同じ不一致を作っていた**

## VERSION 6.15.0 → 6.17.0 — 宣言層の lock-step 清算が完了（v6.17.0 F1-F8 / PR #248 #253 #256 #257 #258、2026-09-06）

**この版を持つと何が違うか**: 「在るのに宣言されていないもの」が CI で落ちるようになる。
これまで宣言層は「宣言 → 実体」（宣言したものが在るか）しか検査しておらず、逆向き・鮮度・実質は
誰も見ていなかった。skill を足して `GRAPH.yml` に書き忘れれば黙って通り、RL を配っても読込経路が
無ければ誰も気づかず、「社外秘」と書いたログが配布先へ配られても検出されなかった。
6.17.0 は宣言層の 8 箇所の乖離を戻し、同じ離れ方が二度起きない機構を置いた。

### 何が検査されるようになったか

`harness-verifier` の**検査 8「宣言被覆」が 13 項目**を持つ（v6.15.0 時点では存在しなかった検査）。

| 検査 | 見るもの |
|---|---|
| 1-6（F1） | `VERSION` ⇄ `GRAPH.yml` の版整合 / upgrade-spec 状態行の値域と実態 / §バージョン履歴の凍結 |
| 7-9（F2） | skill・script が `GRAPH.yml` に宣言されているか / `edge.source` が本当に `edge.to` を指しているか |
| 10（F6） | 共通 RL の実ファイル ⇄ README §common/ の現況（**件数ではなくファイル名**で突合） |
| 11-12（F4） | manifest の分類網羅 / `owned_skills` ⇄ GRAPH nodes ⇄ 実在 dir の三者一致 |
| 13（F3） | 配布物（`overwrite` 配下）に `history/` や `*LOG*.md` を置いていないか |

加えて `frontmatter` 検査（検査 1）が `target_os` を必須にし、`signal-scan` に検知器 (e) `workflow_silence` と
(f) `metabolism_stall` が入った。

### 是正された 8 箇所

| # | 何が起きていたか |
|---|---|
| (a) | `VERSION` 6.11.0 / `GRAPH.version` 6.12.0 / spec 状態行が三重に食い違い、昇格の契機がどこにも定義されていなかった |
| (b) | `GRAPH.yml` に 4 skill が未登録。G-5 が prefix フィルタで `rtk-integration` を**黙って走査対象外**にしていた |
| (c) | 「社外秘ゆえ skill 内部に閉じて保管する」と宣言された COUNCIL-LOG が、`overwrite` 配下ゆえ**配布先 2 リポへ byte 一致で配られていた**。しかも見出し形式でパーサが読めず、そこにしか無い 12 件が 5 ヶ月 CTL 統計から脱落していた |
| (d) | manifest 4 分類に `.claude/agents/` `.gitignore` の行が無く既定 never_touch に落ちていた。さらに正典手順 `rm -rf .claude/skills` は配布先の固有 skill を消すため、**実運用が 8 回連続で使わなかった** |
| (e) | Level A E-3 が「OS 非依存」を要求しながら Windows native を列挙せず、DH 配布の `rtk-integration` が正面から反したまま常駐していた |
| (f) | 共通 RL 6 本が 3 リポへ byte 一致で届いているのに、**読込経路がどこにも無く誰も読んでいなかった** |
| (g) | `GH_REVIEW_PAT` 失効で auto-merge が red。`gemini-review` は 4 ヶ月沈黙していたが、走らなくなったことは red にならないため誰も気づかなかった |
| (h) | 情報代謝が 2026-06-07 で停止（cursor 記録時点から +2,417 行）。根本原因は reindex-protocol の prefix checksum が先頭 append と非互換で guard が構造的に通らないこと |

### 設計上の要点

- **検査器を 8 本作っていない**。静的な網羅性・整合性は `declaration_coverage.py` 1 本に集約し、
  時間経過による停滞は `signal-scan.py` の検知器として既存の枠に寄生させた
  （`ritual-protocol.md`「新しい常駐機構は作らず既存の儀式に寄生する」と同型）
- **allowlist を作っていない**。`graph_excluded` と `owned_skills` は「検査を免除するリスト」ではなく
  「性質・所有の宣言」であり、列挙から漏れたものは免除されるのではなく**配布されない**側に落ちる
- **すべての検査の検出能力を合成ツリーで実証した**。「実リポで PASS した」だけでは空振りを示せないため、
  欠陥を仕込めば検出し健全なら 0 件であることを回帰テストで固定した

### 3 つの Council 判定を経ている

`escalation-matrix.md:31` により規範文書改変は実装前 Council 諮問を要する。
D-3（COUNCIL-LOG の処遇・C3）/ D-4（正典手順の調停・C4）/ D-5（E-3 の改訂形・C1）の 3 件を
2026-09-06 に諮問し、いずれも `auto_agree`。判定は `history/COUNCIL-LOG.md` に記録した。
D-4 / D-5 は 3 軸が**共有トークン 0 の異なる次元**から同じ結論に達している（reason_divergence）。

### 残った留保

**「静かな失敗」** — 配られない・動かない・記録されないという失敗が誰も困らせないまま持続する構造は、
本 spec では閉じない。8 項目のうち 7 項目が静かな失敗であり、**8 項目すべてが「点検」という
単発の外部走査で発見された**（定常的な検知機構が拾ったものは 1 件も無い）。
分析と提案は `delivery/ANALYSIS-silent-failure-2026-09-06.md` に残した。
