...
>
> **転記時の畳み方（2026-09-06 追補・Copilot レビュー指摘の是正）**: 初版は 400 字で機械的に切っており、
> 12 件中 8 件が**単語や識別子の途中**で切れていた（`` `follow_up_ `` が `` `f `` で終わる等）。
> YAML としては閉じていたが、読み手には意味の欠けた断片が残り、しかも続きの所在が示されていなかった。
> 現在は 360 字を上限に**句点・閉じ括弧の直後**で切り、切った場合は末尾に
> 「…（全文は history/council-log-skill-archive.md）」を付して**続きの所在を明示する**。
> 半角ピリオドとカンマを切れ目に含めないのは、`orchestrator.` や `v4.` のような識別子・
> バージョン番号の途中で切れるためである。
