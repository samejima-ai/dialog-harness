...
## 6-A. 比較の相手を作る（2026-08-30 追記）

初版は「W9 は外部と比較不能」で止まっていた。単独の 10.9% は高いのか低いのか決まらないため、
**比較先を外部に求めるのをやめ、2 方向で内側に取った**。

### 横断 — 同じハーネスを積んだ別プロジェクト

`kakuman-platform-v3.0` は DH を利用者側（D3）で積んだ実プロダクト（建設業の業務アプリ）。
ドメインは DH 自身（メタハーネス）と何も共有しない。同じツールがそのまま動く
（`--base origin/main` を渡すだけ）。

| 指標 | DH (D4) | kakuman (D3) | 比 |
|---|---|---|---|
| agent PR 数 | 165 | 539 | — |
| 観測期間 | 133.8 日 | 113.5 日 | — |
| change lead time 中央値 | 18.5 分 | 16.3 分 | **0.88×** |
| lead time 1 日以内 | 98.8% | 98.0% | **0.99×** |
| 復旧時間 中央値 | 0.9 時間 | 1.1 時間 | **1.22×** |
| PR あたり commit | median 2 / mean 2.3 | median 2 / mean 2.3 | **1.00×** |
| deployment rework rate | 10.9% | 3.7% | 0.34× |
| deployment frequency | 1.23/日 | 4.75/日 | 3.86× |
| human-commit 介入率 | 1.8% | 7.6% | 4.22× |

**上 4 行はほぼ一致する。** ドメインが何も共通しない 2 プロジェクトで揃うのだから、これは
**ハーネスの性質**である。**下 3 行は 0.34〜4.22 倍でばらつく。** ばらつくということは
**プロジェクトの違いを拾えている**ということで、指標として働いている証拠になる。

### 時系列 — 自分の過去と比べる

期間の境界は両 repo で同一にした（2026-05-08〜08-31、両者が同時に稼働している範囲）。

| 期間 | DH rework | DH 介入率 | kakuman rework | kakuman 介入率 |
|---|---|---|---|---|
| 05-08〜06-15 | 15.7% (n=70) | 1.4% | 3.5% (n=260) | 5.8% |
| 06-15〜07-25 | 7.1% (n=14) | 0.0% | 5.6% (n=142) | 9.2% |
| 07-25〜08-31 | 7.1% (n=42) | 0.0% | 2.2% (n=137) | 9.5% |

**DH の介入率は消え、kakuman の介入率は増えている。** 同じ opt-out auto-merge の下で正反対に動く。
DH の rework は 15.7% → 7.1% と改善するが、中期は n=14 と小さく振れ幅の解釈には注意が必要。

### §3 の結論を一部訂正する

§3 で「human-commit 介入率は制御点が無いので情報を持たない」と書いた。**これは言い過ぎだった。**

同じ opt-out auto-merge の下でこの指標は 1.8%（DH）と 7.6%（kakuman）で **4.2 倍動く**。
動くなら情報は持っている。ただしそれは dotnet/runtime の 45% が測るものとは**別種の情報**である —
45% は「**必須ゲートを通した量**」、DH / kakuman の値は「**義務がない中で人間が自発的に関与した量**」。
参照系との比較が成立しないという §3 の結論は変わらないが、**指標そのものが無意味なのではない**。

そして自発的関与は、**ハーネス自身の開発（D4）でだけ消滅している**（直近 56 PR で 0 件）。
利用者プロジェクト（D3）では逆に増えている。メタ機構の自己開発ほど人間が見なくなる、
という構図がここに出ている。

### 実装 — 計測経路のバグを 1 件検出した

比較を実装する過程で、`fetch_merged_from_master`（`--states` 不在時の代替経路）が
**squash commit の body を落としていた**ことが判明した。rework の「先行 PR 番号を明示参照」
判定は body を読むため、**kakuman の rework が 3.7% → 0.9% に系統的に過小計上されていた**。

発見できたのは、ツールの出力と独立に書いた集計を突き合わせたからである。判定層のテストは
全通過していた（バグは取得層にあった）。修正と同時に `_git` を差し替える回帰テストを追加した
（テスト 13）。**§5 の「計測経路が指標を作る」がここでも起きている。**
